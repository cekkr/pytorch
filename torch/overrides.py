...
def _get_overridable_functions() -> (
    ...
    tested_namespaces = [
        ...
        ("torch.Tensor", torch.TensorBase, dir(torch.TensorBase)),
    ...
    for namespace_str, namespace, ns_funcs in tested_namespaces:
        for func_name in ns_funcs:
            ...
            if namespace is not torch.TensorBase:
            ...
            func = getattr(namespace, func_name)
            if (
                namespace is torch.TensorBase
                and getattr(object, func_name, None) == func
            ):
                continue
...
def _get_tensor_methods() -> Set[Callable]:
    ...
    methods = set(overridable_funcs[torch.TensorBase])
    return methods
...
def is_tensor_like(inp):
    ...
    True
    """
    return type(inp) is torch.TensorBase or hasattr(inp, "__torch_function__")
