...
class ReferenceQuantizedModule(torch.nn.Module):
    def _init_weight_qparams(self, weight_qparams, device):
        ...
        if self.weight_dtype in [
            ...
            zero_point_dtype = (
                ...
                if isinstance(weight_qparams["zero_point"], torch.TensorBase)
                else torch.int
            )
            w_scale = weight_qparams["scale"]
            w_scale_tensor = (
                w_scale.clone().detach()
                if isinstance(w_scale, torch.TensorBase)
            ...
            w_zp_tensor = (
                w_zp.clone().detach()
                if isinstance(w_zp, torch.TensorBase)
            ...
            if self.weight_qscheme in [
                ...
                w_axis_tensor = (
                    w_axis.clone().detach()
                    if isinstance(w_axis, torch.TensorBase)
    ...
    def get_weight(self):
        ...
        assert isinstance(self.weight_scale, torch.TensorBase)
        assert isinstance(self.weight_zero_point, torch.TensorBase)
...
def _quantize_weight_decomposed(
    ...
    weight_scale: torch.TensorBase,
    weight_zero_point: torch.TensorBase,
    weight_axis: int,
