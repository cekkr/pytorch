...
class FloatFunctional(torch.nn.Module):
    ...
    def add(self, x: TensorBase, y: TensorBase) -> TensorBase:
        r = torch.add(x, y)
    ...
    def add_scalar(self, x: TensorBase, y: float) -> TensorBase:
        r = torch.add(x, y)
    ...
    def mul(self, x: TensorBase, y: TensorBase) -> TensorBase:
        r = torch.mul(x, y)
        r = self.activation_post_process(r)
        return r
    ...
    def mul_scalar(self, x: TensorBase, y: float) -> TensorBase:
        r = torch.mul(x, y)
        # Note: this operation is not observed because the observation is not
        # needed for the quantized op.
        return r
    ...
    def cat(self, x: List[TensorBase], dim: int = 0) -> TensorBase:
    ...
    def add_relu(self, x: TensorBase, y: TensorBase) -> TensorBase:
        r = torch.add(x, y)
    ...
    def matmul(self, x: TensorBase, y: TensorBase) -> TensorBase:
        r = torch.matmul(x, y)
        r = self.activation_post_process(r)
        return r
