...
class EmbeddingPackedParams(torch.nn.Module):
    ...
    @torch.jit.export
    def set_weight(self, weight: torch.TensorBase) -> None:
        if self.dtype in [torch.quint8, torch.quint4x2]:
...
class Embedding(torch.nn.Module):
    ...
    def __init__(
        ...
        sparse: bool = False,
        _weight: Optional[TensorBase] = None,
        dtype=torch.quint8,
    ) -> None:
        super().__init__()
    ...
    def forward(self, indices: TensorBase) -> TensorBase:
    ...
    def set_weight(self, w: torch.TensorBase) -> None:
...
class EmbeddingBag(Embedding):
    ...
    def forward(
        ...
        per_sample_weights: Optional[TensorBase] = None,
        compressed_indices_mapping: Optional[TensorBase] = None,
    ) -> TensorBase:
        if self.dtype == torch.quint4x2:
