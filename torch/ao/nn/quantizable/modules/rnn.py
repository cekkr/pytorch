...
class LSTMCell(torch.nn.Module):
    ...
    def forward(
        self, x: TensorBase, hidden: Optional[Tuple[TensorBase, TensorBase]] = None
...
class _LSTMLayer(torch.nn.Module):
    ...
    def forward(
        ...
        if hidden is None:
            ...
            hx_fw, cx_fw = hidden
        hidden_bw: Optional[Tuple[TensorBase, TensorBase]] = None
        if self.bidirectional:
            if hx_fw is None:
                hx_bw = None
            else:
...
class LSTM(torch.nn.Module):
    ...
    def forward(
        ...
        if hidden is None:
            ...
            if isinstance(hidden_non_opt[0], TensorBase):
