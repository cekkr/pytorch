...
class UniformQuantizationObserverBase(ObserverBase):
    ...
    def _calculate_qparams(
        self, min_val: torch.TensorBase, max_val: torch.TensorBase
...
class MinMaxObserver(UniformQuantizationObserverBase):
    ...
    min_val: torch.TensorBase
    max_val: torch.TensorBase
...
class PerChannelMinMaxObserver(UniformQuantizationObserverBase):
    ...
    def _load_from_state_dict(
        ...
        prefix: str,
        local_metadata: Dict[str, torch.TensorBase],
        strict: bool,
...
class HistogramObserver(UniformQuantizationObserverBase):
    ...
    def _get_norm(
        self,
        delta_begin: torch.TensorBase,
        delta_end: torch.TensorBase,
        density: torch.TensorBase,
    ) -> torch.TensorBase:
        r"""
    ...
    def _non_linear_param_search(self) -> Tuple[torch.TensorBase, torch.TensorBase]:
    ...
    def _adjust_min_max(
        self,
        combined_min: torch.TensorBase,
        combined_max: torch.TensorBase,
        upsample_rate: int,
    ) -> Tuple[torch.TensorBase, torch.TensorBase, int, int]:
        # We ensure that:
        ...
        downsample_rate = int(
            torch.ceil(
                ((combined_max - combined_min) / (self.max_val - self.min_val))
                * upsample_rate
            ).item()
        )
        e = downsample_rate / upsample_rate * (self.max_val - self.min_val) - (
    ...
    def _combine_histograms(
        self,
        orig_hist: torch.TensorBase,
        new_hist: torch.TensorBase,
        upsample_rate: int,
        downsample_rate: int,
        start_idx: int,
        Nbins: int,
    ...
    def forward(self, x_orig: torch.TensorBase) -> torch.TensorBase:
        if x_orig.numel() == 0:
            return x_orig
        x = x_orig.detach()
...
class RecordingObserver(ObserverBase):
    ...
    __annotations__ = {"tensor_val": List[Optional[torch.TensorBase]]}
