...
def reshape_scale(
    scale: torch.TensorBase, axis: int, input: torch.TensorBase
) -> torch.TensorBase:
...
def scale_weight_node(
    ...
    equalization_scale: torch.TensorBase,
    next_equalization_scale: Optional[torch.TensorBase],
) -> None:
    ...
    assert isinstance(weight, torch.TensorBase)
    ...
    if bias is None:
        return
    assert isinstance(bias, torch.TensorBase)
...
def get_layer_sqnr_dict(
    model_a: nn.Module, model_b: nn.Module, x: torch.TensorBase
) -> Dict[str, float]:
