...
RNNReturnType = Tuple[torch.TensorBase, Tuple[torch.TensorBase, torch.TensorBase]]
...
class OutputLogger(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.stats: List[torch.TensorBase] = []
...
class OutputComparisonLogger(OutputLogger):
    ...
    def forward(self, x, x_ref):
        ...
        if not self.enabled:
            return x
        assert isinstance(x, torch.TensorBase), "non-tensor inputs not yet supported"
...
def extend_logger_results_with_comparison(
    ...
    model_name_1: str,
    model_name_2: str,
    comparison_fn: Callable[[torch.TensorBase, torch.TensorBase], torch.TensorBase],
    comparison_name: str,
) -> None:
    """
