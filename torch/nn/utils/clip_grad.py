...
_tensor_or_tensors = Union[torch.TensorBase, Iterable[torch.TensorBase]]
...
def clip_grad_norm_(
    ...
    """
    if isinstance(parameters, torch.TensorBase):
    ...
    grouped_grads: Dict[
        Tuple[torch.device, torch.dtype], Tuple[List[List[TensorBase]], List[int]]
    ] = _group_tensors_by_device_and_dtype(
        [grads]
    )  # type: ignore[assignment]

    norms: List[TensorBase] = []
...
def clip_grad_value_(
    ...
    for (device, _), ([grads], _) in grouped_grads.items():  # type: ignore[assignment]
        if (
            foreach is None
            and _has_foreach_support(cast(List[TensorBase], grads), device=device)
        ) or (foreach and _device_has_foreach_support(device)):
            torch._foreach_clamp_min_(cast(List[TensorBase], grads), -clip_value)
            torch._foreach_clamp_max_(cast(List[TensorBase], grads), clip_value)
        elif foreach:
            raise RuntimeError(
                f"foreach=True was passed, but can't use the foreach API on {device.type} tensors"
            )
            ...
            for grad in grads:
                cast(TensorBase, grad).clamp_(min=-clip_value, max=clip_value)
