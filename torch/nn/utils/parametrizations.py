...
class _Orthogonal(Module):
    ...
    def right_inverse(self, Q: torch.TensorBase) -> torch.TensorBase:
...
def orthogonal(
    ...
    *,
    ...
    if not isinstance(weight, TensorBase):
        raise ValueError(
...
class _SpectralNorm(Module):
    ...
    def _reshape_weight_to_matrix(self, weight: torch.TensorBase) -> torch.TensorBase:
        # Precondition
    ...
    @torch.autograd.no_grad()
    def _power_method(
        self, weight_mat: torch.TensorBase, n_power_iterations: int
    ) -> None:
    ...
    def forward(self, weight: torch.TensorBase) -> torch.TensorBase:
        if weight.ndim == 1:
    ...
    def right_inverse(self, value: torch.TensorBase) -> torch.TensorBase:
