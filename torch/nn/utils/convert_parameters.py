...
def parameters_to_vector(parameters: Iterable[torch.TensorBase]) -> torch.TensorBase:
...
def vector_to_parameters(
    vec: torch.TensorBase, parameters: Iterable[torch.TensorBase]
) -> None:
    ...
    if not isinstance(vec, torch.TensorBase):
...
def _check_param_device(
    param: torch.TensorBase, old_param_device: Optional[int]
) -> int:
