...
class _ConvNd(Module):
    ...
    __annotations__ = {"bias": Optional[torch.TensorBase]}

    def _conv_forward(self, input: TensorBase, weight: TensorBase, bias: Optional[TensorBase]) -> TensorBase:  # type: ignore[empty-body]
        ...
...
class Conv1d(_ConvNd):
    ...
    * :attr:`dilation` controls the spacing between the kernel points; also
    ...
    def _conv_forward(
        self, input: TensorBase, weight: TensorBase, bias: Optional[TensorBase]
    ):
        if self.padding_mode != "zeros":
            return F.conv1d(
                F.pad(
...
class ConvTranspose1d(_ConvTransposeNd):
    ...
    * :attr:`dilation` controls the spacing between the kernel points; also known as the à trous algorithm.
    ...
    def forward(
        self, input: TensorBase, output_size: Optional[List[int]] = None
    ) -> TensorBase:
        if self.padding_mode != "zeros":
            raise ValueError(
...
class _LazyConvXdMixin(LazyModuleMixin):
    ...
    def _get_in_channels(self, input: TensorBase) -> int:
