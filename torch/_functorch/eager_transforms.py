...
def _is_differentiable(maybe_tensor):
    if not isinstance(maybe_tensor, torch.TensorBase):
        return False
    ...
    with grad_increment_nesting() as level:
        ...
        with torch.enable_grad():
            ...
            for primal_out in flat_primals_out:
                assert isinstance(primal_out, torch.TensorBase)
...
def error_if_complex(func_name, args, is_input):
    ...
    for idx, arg in enumerate(flat_args):
        if isinstance(arg, torch.TensorBase) and arg.dtype.is_complex:
...
def assert_flat_tuple_of_tensors(elts: Any, api: str, argname: str) -> None:
    ...
    for elt in elts:
        if isinstance(elt, torch.TensorBase):
            continue
        raise RuntimeError(
...
def assert_output_is_tensor_or_tensors(output: Any, api: str) -> None:
    if isinstance(output, torch.TensorBase):
        return
...
def assert_non_empty_list_of_tensors(
    output: List[torch.TensorBase], api: str, argname: str
) -> None:
    if len(output) == 0:
...
def safe_unpack_dual(dual, strict):
    if not isinstance(dual, torch.TensorBase):
        raise RuntimeError(
...
def grad_and_value_impl(func, argnums, has_aux, args, kwargs) -> Callable:
    with grad_increment_nesting() as level:
        ...
        with torch.enable_grad():
            ...
            if not isinstance(output, torch.TensorBase):
                raise RuntimeError(
    ...
    def wrapped(*args, **kwargs):
        try:
            ...
            for unwrapped, wrapped in zip(
                ...
                if isinstance(unwrapped, torch.TensorBase) and isinstance(
                    wrapped, torch.TensorBase
                ):
    ...
    def trace_fn(flat_tangents):
        with fwAD.dual_level():
            ...
            tangents = tree_map_only(
                torch.TensorBase, lambda t: fwAD.unpack_dual(t)[1], output
            )
