    ...
    if "val" in node.meta:
        ...
        # layering violation)
            ...
            return sum(
                ...
                for n in val
                if isinstance(n, torch.TensorBase)
            )
...
def functionalize_rng_ops(joint_module, fw_module, bw_module, num_sym_nodes):
    ...
    def get_device(node):
        ...
        for candidate in candidates:
            if isinstance(candidate, torch.TensorBase):
...
def min_cut_rematerialization_partition(
    ...
    *,
    ...
    orig_fw_outputs, required_fw_nodes, required_bw_nodes, unclaimed_nodes, inputs = (
    ...
    for node in full_bw_graph.nodes:
        ...
        ) or ("val" in node.meta and not isinstance(node.meta["val"], torch.TensorBase))
