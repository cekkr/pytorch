...
def _maybe_remove_batch_dim(name, batched_output, vmap_level, batch_size, out_dim):
    ...
    if out_dim is None:
        if isinstance(batched_output, torch.TensorBase) and is_batchedtensor(
            batched_output
        ):
            raise ValueError(
    ...
    # out_dim is non None
    if not isinstance(batched_output, torch.TensorBase):
        raise ValueError(
...
def _unwrap_batched(
    ...
    if isinstance(batched_outputs, torch.TensorBase):
