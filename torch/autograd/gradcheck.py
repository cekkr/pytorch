...
def _is_sparse_compressed_tensor(obj: torch.TensorBase):
...
def _is_sparse_any_tensor(obj: torch.TensorBase):
...
def _iter_tensors(
    x: Union[torch.TensorBase, Iterable[torch.TensorBase]],
...
def _get_numerical_jacobian(
    ...
    **very careful** in this to not clone `target`.
    """
    jacobians: List[Tuple[torch.TensorBase, ...]] = []
    if outputs is None:
...
def _combine_jacobian_cols(
    jacobians_cols: Dict[int, List[torch.TensorBase]], outputs, input, numel
) -> Tuple[torch.TensorBase, ...]:
...
def _prepare_input(
    ...
    maybe_perturbed_input: Optional[torch.TensorBase],
    fast_mode=False,
) -> torch.TensorBase:
...
def get_numerical_jacobian_wrt_specific_input(
    ...
    jacobian_cols: Dict[int, List[torch.TensorBase]] = {}
...
def _get_numerical_vJu(
    ...
    reduced_jacobians: List[List[torch.TensorBase]] = []
    for i, (inp_idx, u) in enumerate(zip(inp_indices, all_u)):
        ...
        if all_v is not None:
            jacobian_scalars: List[torch.TensorBase] = []
...
) -> Tuple[Tuple[torch.TensorBase, ...], bool, bool]:
...
def _compute_analytical_jacobian_rows(
...
) -> List[List[Optional[torch.TensorBase]]]:
    ...
    jacobians_rows: List[List[Optional[torch.TensorBase]]] = []
    for j in range(flat_grad_out.numel()):
        ...
        for i, d_x in enumerate(grad_inputs):
            ...
            jacobians_rows[i] += [
                d_x.clone() if isinstance(d_x, torch.TensorBase) else None
            ]
...
def _get_analytical_vjps_wrt_specific_output(
    ...
    vjps: List[List[Optional[torch.TensorBase]]] = []
    grad_inputs = vjp_fn(v.reshape(sample_output.shape))
    for vjp in grad_inputs:
        vjps.append([vjp.clone() if isinstance(vjp, torch.TensorBase) else None])
    return vjps
...
def _check_outputs(outputs) -> None:
    if any(
        _is_sparse_any_tensor(t) for t in outputs if isinstance(t, torch.TensorBase)
    ):
    ...
    if any(t.layout == torch._mkldnn for t in outputs if isinstance(t, torch.TensorBase)):  # type: ignore[attr-defined]
        raise ValueError(
...
def _test_batched_grad_forward_ad(func, inputs) -> bool:
    ...
    for input_idx, current_input in enumerate(inputs):
        ...
        def jvp(tangent: torch.TensorBase):
            with fwAD.dual_level():
...
def _test_backward_mul_by_grad_output(outputs, inputs, masked) -> bool:
    ...
    diff_input_list: List[torch.TensorBase] = list(_iter_tensors(inputs, True))
    ...
    for gi, di in zip(grads_input, diff_input_list):
        if gi is None:
            continue
        if isinstance(gi, torch.TensorBase) and gi.layout != torch.strided:
