...
TensorListList: TypeAlias = List[List[Optional[TensorBase]]]
...
def _has_foreach_support(tensors: List[TensorBase], device: torch.device) -> bool:
    return _device_has_foreach_support(device) and all(
        t is None or type(t) == torch.TensorBase for t in tensors
    )
