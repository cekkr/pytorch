...
def _generate_tensor_methods_for_privateuse1_backend(custom_backend_name: str) -> None:
    ...
    def wrap_tensor_backend(self: torch.TensorBase) -> bool:
        return self.device.type == custom_backend_name

    _check_register_once(torch.TensorBase, f"is_{custom_backend_name}")
    setattr(torch.TensorBase, f"is_{custom_backend_name}", wrap_tensor_backend)
    ...
    _check_register_once(torch.TensorBase, custom_backend_name)
    setattr(torch.TensorBase, custom_backend_name, wrap_tensor_to)
...
def _generate_module_methods_for_privateuse1_backend(custom_backend_name: str) -> None:
    ...
    if not hasattr(torch.TensorBase, custom_backend_name):
        raise RuntimeError(
