...
def _tensor_to_half_val(t: torch.TensorBase) -> List[int]:
    return [half_to_int(x) for x in t.flatten().tolist()]


def _tensor_to_complex_val(t: torch.TensorBase) -> List[float]:
    return torch.view_as_real(t).flatten().tolist()


def _tensor_to_list(t: torch.TensorBase) -> List[Any]:
