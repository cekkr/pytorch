...
def autograd_not_implemented(custom_op):
    def kernel(*args, **kwargs):
        if torch.is_grad_enabled() and pytree.tree_any(
            lambda x: isinstance(x, torch.TensorBase) and x.requires_grad,
            (args, kwargs),
        ):
...
def save_pytree_for_backward(ctx, stuff):
    ...
    tensor_idxs = [
        ...
        if isinstance(thing, torch.TensorBase)
    ]
    non_tensor_idxs = [
        idx
        for idx, thing in enumerate(flat_stuff)
        if not isinstance(thing, torch.TensorBase)
    ]
    tensors = [thing for thing in flat_stuff if isinstance(thing, torch.TensorBase)]
    non_tensors = [
        thing for thing in flat_stuff if not isinstance(thing, torch.TensorBase)
    ]
