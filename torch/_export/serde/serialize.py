...
def serialize_tensor_meta(t: torch.TensorBase) -> TensorMeta:
    """
...
class GraphModuleSerializer:
    ...
    def serialize_input(self, arg) -> Argument:
        ...
        if isinstance(arg, torch.fx.Node):
            if arg.op == "get_attr":
                ...
                if isinstance(attr, torch.TensorBase):
                    raise SerializeError(
        ...
        if isinstance(meta_val, tuple):
            ...
            if len(meta_val) == 1:
                ...
                for i, v in enumerate(meta_val):
                    assert isinstance(v, torch.TensorBase)
    ...
    def serialize_output(self, name: str, meta_val: Any) -> Argument:
        ...
        if meta_val is None:
            return Argument.create(as_none=())
        if isinstance(meta_val, torch.TensorBase):
            # e.g "-> Tensor"
            return Argument.create(
...
class ExportedProgramDeserializer:
    ...
    def deserialize(
        ...
        if (
            serialized_artifact.exported_program.schema_version.major
            != SCHEMA_VERSION[0]
        ):
            raise SerializeError(
