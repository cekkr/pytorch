...
import torch
from torch import SymInt, TensorBase
...
def as_nested_tensor(
    tensor_list: Sequence[TensorBase],
    ...
    if not isinstance(tensor_list, list) or any(
        not isinstance(t, TensorBase) for t in tensor_list
    ):
        raise TypeError(
...
def nested_tensor(
    ...
    *,
    ...
    if layout == torch.strided:
        ...
        list_of_tensors = [
            t.detach() if isinstance(t, TensorBase) else torch.as_tensor(t)
...
def narrow(
    tensor: TensorBase,
    dim: int,
    start: Union[int, TensorBase],
    length: Union[int, TensorBase],
    layout=torch.strided,
) -> TensorBase:
    r"""
    ...
    Example::
        ...
        False
    """
    if not isinstance(start, (int, SymInt, TensorBase)):
        raise RuntimeError("start must be an integer or a tensor")

    if not isinstance(length, (int, SymInt, TensorBase)):
        raise RuntimeError("length must be an integer or a tensor")

    if layout == torch.strided:
        if isinstance(start, TensorBase) or isinstance(length, TensorBase):
            raise RuntimeError(
