...
class FakeTensorProp(torch.fx.Interpreter):
    ...
    def run_node(self, n: Node):
        ...
        if (
            "val" in n.meta
            and isinstance(v := n.meta["val"], torch.SymInt)
            and isinstance(v.node.expr, sympy.Symbol)
            and free_unbacked_symbols(v)
        ):
            sym = v
    ...
    def propagate(self, *args):
        fake_args = [
            self._mode.from_tensor(a) if isinstance(a, torch.TensorBase) else a
            for a in args
        ]
