...
def has_proxy_slot(obj, tracer):
    assert isinstance(obj, (torch.TensorBase, SymNode)), type(obj)
...
def set_meta(proxy, val):
    ...
    elif isinstance(val, torch.TensorBase) and not val.is_sparse:
        proxy.node.meta["tensor_meta"] = _extract_tensor_metadata(val)
    return proxy
...
HANDLED_TYPES = (torch.TensorBase, torch.nn.Parameter, FakeTensor)
...
def proxy_call(proxy_mode, func, pre_dispatch, args, kwargs):
    ...
    if not pytree.tree_all_only(torch.TensorBase, can_handle_tensor, (args, kwargs)):
    ...
    f_args, f_kwargs = pytree.tree_map_only(
        (torch.TensorBase, torch.ScriptObject),
    ...
    if torch.Tag.data_dependent_output in func.tags:
        ...
        if proxy_mode._error_on_data_dependent_ops and pytree.tree_all_only(
            torch.TensorBase, lambda t: not is_fake(t), (args, kwargs)
        ):
            raise RuntimeError(
    ...
    if (
        ...
        and any_constant
        and pytree.tree_all_only(
            torch.TensorBase, lambda t: t.numel() <= CONSTANT_NUMEL_LIMIT, out
        )
    ):
