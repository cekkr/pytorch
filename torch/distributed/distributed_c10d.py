...
class _CollOp:
    ...
    def __init__(
        ...
        op: Callable,
        tensor: torch.TensorBase,
        dst_tensor: Optional[torch.TensorBase] = None,
...
def _check_tensor_list(param, param_name) -> None:
    ...
    elif not all(isinstance(p, torch.TensorBase) for p in param):
        raise TypeError(
            f"""Invalid function argument. Expected parameter `{param_name}` of type List[torch.Tensor]
...
def _object_to_tensor(obj, device, group):
    ...
    if get_debug_level() == DebugLevel.DETAIL and is_nccl_available():
        ...
        if backend == Backend.NCCL:
            ...
            logger.warning(
                f"_object_to_tensor size: {byte_tensor.numel()} hash value: {hash}"
            )  # noqa: G004
...
def _tensor_to_object(tensor, tensor_size, group):
    if get_debug_level() == DebugLevel.DETAIL and is_nccl_available():
        ...
        if backend == Backend.NCCL:
            ...
            logger.warning(
                f"_tensor_to_object size: {tensor.numel()} hash value: {hash}"
            )  # noqa: G004
    tensor = tensor.cpu()
