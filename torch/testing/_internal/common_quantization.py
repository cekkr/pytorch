                    ...
                    for res_idx in range(len(layer_data[model_name_0])):
                        ...
                        if not is_weight_functional_conv1d:
                            for idx in range(len(layer_data_0["values"])):
                                ...
                                if isinstance(values_0, torch.TensorBase):
                                    self.assertTrue(
...
class QuantizationLiteTestCase(QuantizationTestCase):
    ...
    def _compare_script_and_mobile(
        self, model: torch.nn.Module, input: torch.TensorBase
    ):
...
class ManualEmbeddingBagLinear(nn.Module):
    ...
    def forward(
        ...
        offsets: Optional[torch.TensorBase] = None,
        per_sample_weights: Optional[torch.TensorBase] = None,
    ):
...
class DenseTopMLP(nn.Module):
    ...
    def forward(
        self,
        sparse_feature: torch.TensorBase,
...
class SparseNNModel(nn.Module):
    ...
    def forward(
        self,
        sparse_indices: torch.TensorBase,
        sparse_offsets: torch.TensorBase,
        dense: torch.TensorBase,
    ) -> torch.TensorBase:
