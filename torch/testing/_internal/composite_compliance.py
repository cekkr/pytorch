...
def check_metadata_consistency(wrapper_tensor, CCT):
    ...
    if not isinstance(wrapper_tensor, CCT):
        return
    things_to_check = {
        "shape": TensorBase.size,
        "dtype": lambda x: x.dtype,
        "device": lambda x: x.device,
        "numel": TensorBase.numel,
        "stride": TensorBase.stride,
        "storage_offset": TensorBase.storage_offset,
    }
...
def generate_cct_and_mode(autograd_view_consistency=True):
    ...
    class CompositeCompliantTensor(torch.TensorBase):
...
def is_tensorlist(lst):
    ...
    if len(lst) == 0:
        return False
    all_tensors = all(isinstance(elt, torch.TensorBase) for elt in lst)
    if all_tensors:
        return True
    exists_one_tensor = all(isinstance(elt, torch.TensorBase) for elt in lst)
...
def generate_subclass_choices(flat_args, CCT, cct_mode):
    # CCT: CompositeCompliantTensor class which is generated using generate_cct_and_mode
    is_tensor_likes = [
        isinstance(arg, torch.TensorBase) or is_tensorlist(arg) for arg in flat_args
    ]
    subclass_options = [
...
def check_with_mode(op, args, kwargs, assert_equal_fn):
    ...
    def wrap(e):
        return CCT(e, cct_mode) if isinstance(e, torch.TensorBase) else e
...
def compute_expected_grads(
    ...
    flat_results = [r for r in flat_results if isinstance(r, torch.TensorBase)]
...
def check_forward_ad_formula(
    ...
    with fwAD.dual_level():
        ...
        def maybe_make_dual(dual):
            ...
            primal, tangent = dual
            if isinstance(primal, torch.TensorBase) and primal.requires_grad:
