...
def do_input_map(fn, input):
    return _nested_map(lambda t: isinstance(t, torch.TensorBase), fn)(input)
        ...
        with torch.jit.optimized_execution(optimize):
            with enable_profiling_mode_for_profiling_tests():
                extra_profile_runs = any(
                    isinstance(x, torch.TensorBase) and x.requires_grad for x in inputs
                )
                if isinstance(script, str):
...
def clone_inputs(args):
    inputs: List[Union[torch.TensorBase, List[torch.TensorBase]]] = []
...
def get_traced_sample_variant_pairs(device, dtype, op):
    ...
    if has_fake_function:
        variants = {"method": getattr(torch.TensorBase, op.name)}
