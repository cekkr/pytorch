...
def is_abstract(tensor: torch.TensorBase) -> bool:
    if tensor.is_meta:
        return True
...
def safe_schema_check(
    ...
    *,
    ...
    if pytree.tree_any_only(torch.TensorBase, is_abstract, (args, kwargs)):
        return None
...
def safe_autograd_registration_check(
    ...
    *,
    ...
    if not pytree.tree_any_only(
        torch.TensorBase, lambda x: x.requires_grad, (args, kwargs)
    ):
        return
    ...
    *,
    ...
    def func(*args, **kwargs):
        args, kwargs = pytree.tree_map_only(
            torch.TensorBase, torch.clone, (args, kwargs)
        )
...
def deepcopy_tensors(inputs: Any) -> Any:
    return pytree.tree_map_only(torch.TensorBase, clone_input, inputs)
