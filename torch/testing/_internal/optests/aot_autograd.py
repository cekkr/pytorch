...
def aot_autograd_check(
    ...
    args_is_tensor = [isinstance(arg, torch.TensorBase) for arg in flat_args]
    ...
    if check_gradients == "auto":
        any_tensor_requires_grad = pytree.tree_any_only(
            torch.TensorBase, lambda x: x.requires_grad, args
        )
        any_output_requires_grad = pytree.tree_any_only(
            torch.TensorBase, lambda x: x.requires_grad, out
        )
    ...
    def call_forwards_backwards(f, args):
        ...
        for i in flat_out:
            if isinstance(i, torch.TensorBase):
                ...
                sm += i.sum().abs()
        assert isinstance(sm, torch.TensorBase)
