...
def single_batch_reference_fn(input, parameters, module):
    ...
    single_batch_input = (
        [single_batch_input]
        if isinstance(single_batch_input, torch.TensorBase)
...
class NNTestCase(TestCase):
    ...
    def _backward(
        ...
        grad_output: Union[torch.TensorBase, Sequence[torch.TensorBase]],
        ...
        if name not in self._arg_cache:
            ...
            if name in self._extra_kwargs:
                ...
                def map_tensor_sizes(sizes):
                    ...
                    elif isinstance(sizes, torch.TensorBase):
                        return sizes.double()
                    else:
...
class ModuleTest(TestBase):
    ...
    def test_noncontig(self, test_case, module, input):
        ...
        if isinstance(input, torch.TensorBase) and input.dim() == 0:
            return
        if any(i.dim() == 0 for i in input if isinstance(i, torch.TensorBase)):
            return
    ...
    def test_cuda(self, test_case):
        ...
        with set_default_dtype(self.default_dtype):
            ...
            is_any_input_complex = any(
                isinstance(t, torch.TensorBase) and t.dtype.is_complex
...
class InputVariableMixin:
    def _get_input(self):
        ...
        def map_variables(i):
            if isinstance(i, torch.TensorBase):
