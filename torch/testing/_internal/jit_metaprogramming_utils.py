...
def create_input(
    ...
    def map_arg(arg):
        ...
        if isinstance(arg, (torch.Size, dont_convert)):
            ...
            return var
        elif isinstance(arg, tuple) and not isinstance(arg[0], torch.TensorBase):
            return conjugate(
                maybe_non_contig(torch.randn(*arg, dtype=dtype, device=device))
            ).requires_grad_(requires_grad)
        # double check casting
        elif isinstance(arg, non_differentiable):
            if isinstance(arg.tensor, torch.TensorBase):
...
def get_script_args(args):
    formals: List[str] = []
    tensors: List[Union[torch.TensorBase, List[torch.TensorBase]]] = []
    actuals: List[str] = []
    for arg in args:
...
class SplitInputs:
    ...
    def _is_tensor_input(arg):
        return isinstance(arg, torch.TensorBase) or is_iterable_of_tensors(arg)
