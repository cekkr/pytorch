...
def xpassIfTorchDynamo(func):
    return (
        func if TEST_WITH_TORCHDYNAMO else unittest.expectedFailure(func)
    )  # noqa: F821


def xfailIfTorchDynamo(func):
    return (
        unittest.expectedFailure(func) if TEST_WITH_TORCHDYNAMO else func
    )  # noqa: F821
...
def is_iterable_of_tensors(iterable, include_empty=False):
    ...
    if isinstance(iterable, torch.TensorBase):
        return False
...
try:
    ...
    hypothesis.settings.load_profile(
        "pytorch_ci"
        if IS_CI
        else os.getenv("PYTORCH_HYPOTHESIS_PROFILE", "dev")  # noqa: F821
    )
except ImportError:
...
class RelaxedBooleanPair(BooleanPair):
    ...
    def _process_inputs(self, actual, expected, *, id):
        ...
        tensor_or_array_types: Tuple[Type, ...] = (torch.TensorBase, np.ndarray)
    ...
    def _to_bool(self, bool_like, *, id):
        ...
        elif isinstance(bool_like, (torch.TensorBase, np.ndarray)):
            numel = (
                bool_like.numel()
                if isinstance(bool_like, torch.TensorBase)
                else bool_like.size
            )
            if numel > 1:
                self._fail(
...
class RelaxedNumberPair(NumberPair):
    ...
    def _to_number(self, number_like, *, id):
        if isinstance(number_like, (torch.TensorBase, np.ndarray)):
            numel = (
                number_like.numel()
                if isinstance(number_like, torch.TensorBase)
...
class TensorOrArrayPair(TensorLikePair):
    ...
    def _process_inputs(self, actual, expected, *, id, allow_subclasses):
        self._check_inputs_isinstance(
            actual, expected, cls=(torch.TensorBase, np.ndarray)
        )
...
This message can be suppressed by setting PYTORCH_PRINT_REPRO_ON_FAILURE=0"""
    ...
    def _run_custom(self, result=None):
        ...
        with unittest.mock.patch(
            ...
            if TEST_WITH_TORCHINDUCTOR:  # noqa: F821
                ...
                super_run = torch._dynamo.optimize("aot_eager_decomp_partition")(
                    super_run
                )
            elif TEST_WITH_TORCHDYNAMO:  # noqa: F821
                # TorchDynamo optimize annotation
                super_run = torch._dynamo.optimize("eager", nopython=nopython)(
                    super_run
                )
                ...
                def expect_failure(f, test_name):
                    ...
                    def wrapper(*args, **kwargs):
                        try:
                            ...
                            self.skipTest(e)
                        raise RuntimeError(
                            f"Unexpected success, please remove `test/dynamo_expected_failures/{test_name}`"
                        )
                ...
                def ignore_failure(f, test_name):
                    ...
                    def wrapper(*args, **kwargs):
                        ...
                        if getattr(method, "__unittest_expecting_failure__", False):
                            ...
                            self.skipTest(
                                f"This test passed, maybe we can remove `test/dynamo_skips/{test_name}`"
                            )
    ...
    def assertEqualBroadcasting(self, x, y, *args, **kwargs) -> None:
        ...
        if not isinstance(y, torch.TensorBase):
        ...
        *,
        ...
        if any(
            ...
            def to_list(input):
                return (
                    input.tolist()
                    if isinstance(input, (torch.TensorBase, np.ndarray))
                    else list(input)
                )
        ...
        elif isinstance(x, torch.TensorBase) and isinstance(y, Sequence):
            y = torch.as_tensor(y, dtype=x.dtype, device=x.device)
        elif isinstance(x, Sequence) and isinstance(y, torch.TensorBase):
        ...
        if isinstance(x, torch.TensorBase) and x.is_nested:
            x = x.unbind()
        if isinstance(y, torch.TensorBase) and y.is_nested:
            y = y.unbind()
...
def get_tensors_from(args, kwargs):
    ...
    return set(
        [arg for arg in args if isinstance(arg, TensorBase)]
        + [v for v in kwargs.values() if isinstance(v, TensorBase)]
    )
...
def outs_and_grads(fn, graph_inps, inps):
    ...
    for out in pytree.tree_leaves(outs):
        if isinstance(out, torch.TensorBase) and out.requires_grad:
        ...
        *,
        ...
        for sample in samples:
            ...
            def _input_recomposition_helper(inputs, inp, input_idx):
                ...
                elif isinstance(inp, torch.TensorBase) and inp.requires_grad:
