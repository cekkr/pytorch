...
torch_function_passthrough = {
    ...
    torch.sym_ite,
    torch.TensorBase.dim,
    torch.TensorBase.ndim.__get__,  # type: ignore[attr-defined]
    torch.TensorBase.numel,
    torch.TensorBase.size,
    torch.TensorBase.storage_offset,
    torch.TensorBase.stride,
    torch.TensorBase.dtype.__get__,  # type: ignore[attr-defined]
    torch.TensorBase.is_sparse.__get__,  # type: ignore[attr-defined]
    torch.TensorBase.shape.__get__,  # type: ignore[attr-defined]
    torch.TensorBase.device.__get__,  # type: ignore[attr-defined]
    torch.TensorBase.requires_grad.__get__,  # type: ignore[attr-defined]
    torch.TensorBase.layout.__get__,  # type: ignore[attr-defined]
    torch.TensorBase.is_contiguous,
    # For TorchRefsMode only
    torch.TensorBase.__format__,
    torch.TensorBase.__repr__,
    torch.TensorBase.requires_grad.__get__,  # type: ignore[attr-defined]
}


TensorLikeType = torch.TensorBase
TensorLike = torch.TensorBase
...
def get_dtype(x: Union[torch.TensorBase, NumberType]):
