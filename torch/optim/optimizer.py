...
_foreach_supported_types = [torch.TensorBase, torch.nn.parameter.Parameter]
...
def _dispatch_sqrt(
    ...
    if not torch.jit.is_scripting() and isinstance(x, torch.TensorBase):
        return x.sqrt()
    else:
        return math.sqrt(x)
...
def _default_to_fused_or_foreach(
    params: List[torch.TensorBase], differentiable: bool, use_fused: bool = False
) -> Tuple[bool, bool]:
...
ParamsT: TypeAlias = Union[Iterable[torch.TensorBase], Iterable[Dict[str, Any]]]
...
class Optimizer:
    ...
    def __init__(self, params: ParamsT, defaults: Dict[str, Any]) -> None:
        ...
        self.state: DefaultDict[torch.TensorBase, Any] = defaultdict(dict)
    ...
    def state_dict(self) -> StateDict:
        ...
        * ``param_groups``: a List containing all parameter groups where each
        ...
        packed_state = {
            (param_mappings[id(k)] if isinstance(k, torch.TensorBase) else k): v
    ...
    def load_state_dict(self, state_dict: StateDict) -> None:
        ...
        state: DefaultDict[torch.TensorBase, Dict[Any, Any]] = defaultdict(dict)
    ...
    def zero_grad(self, set_to_none: bool = True) -> None:
        ...
        per_device_and_dtype_grads: Optional[
            DefaultDict[torch.device, DefaultDict[torch.dtype, List[torch.TensorBase]]]
        ]
        if foreach:
    ...
    def add_param_group(self, param_group: Dict[str, Any]) -> None:
        ...
        param_set: Set[torch.TensorBase] = set()
        for group in self.param_groups:
