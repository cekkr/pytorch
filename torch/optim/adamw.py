...
class AdamW(Optimizer):
    def __init__(
        ...
        *,
        ...
        if isinstance(lr, TensorBase) and foreach and not capturable:
            raise ValueError(
    ...
    def _init_group(
        ...
        for p in group["params"]:
            ...
            if (
                group["foreach"]
                and isinstance(group["lr"], TensorBase)
...
def adamw(
    ...
    exp_avgs: List[TensorBase],
    exp_avg_sqs: List[TensorBase],
    max_exp_avg_sqs: List[TensorBase],
    ...
    differentiable: bool = False,
    fused: Optional[bool] = None,
    grad_scale: Optional[TensorBase] = None,
    found_inf: Optional[TensorBase] = None,
    has_complex: bool = False,
    *,
    amsgrad: bool,
    beta1: float,
    beta2: float,
    lr: Union[float, TensorBase],
    weight_decay: float,
    eps: float,
    maximize: bool,
):
    ...
    if not torch._utils.is_compiling() and not all(
        isinstance(t, torch.TensorBase) for t in state_steps
    ):
        raise RuntimeError(
    ...
    if fused is None and foreach is None:
        ...
        if foreach and isinstance(lr, TensorBase) and not capturable:
            foreach = False
    if fused is None:
        fused = False
    if foreach is None:
...
def _single_tensor_adamw(
    ...
    max_exp_avg_sqs: List[TensorBase],
    state_steps: List[TensorBase],
    grad_scale: Optional[TensorBase],
    found_inf: Optional[TensorBase],
    *,
    amsgrad: bool,
    beta1: float,
    beta2: float,
    lr: Union[TensorBase, float],
    weight_decay: float,
    eps: float,
    maximize: bool,
    capturable: bool,
...
def _multi_tensor_adamw(
    ...
    *,
    ...
    if isinstance(lr, TensorBase) and not capturable:
        raise RuntimeError(
            "lr as a Tensor is not supported for capturable=False and foreach=True"
        )
...
def _fused_adamw(
    ...
    *,
    ...
    lr_dict = (
        {lr.device: lr}
        if isinstance(lr, TensorBase) and str(lr.device) != "cpu"
        else None
    )
