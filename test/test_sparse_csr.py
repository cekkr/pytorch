...
sparse_compressed_indices_methods = {
    torch.sparse_csr: (torch.TensorBase.crow_indices, torch.TensorBase.col_indices),
    torch.sparse_csc: (torch.TensorBase.ccol_indices, torch.TensorBase.row_indices),
    torch.sparse_bsr: (torch.TensorBase.crow_indices, torch.TensorBase.col_indices),
    torch.sparse_bsc: (torch.TensorBase.ccol_indices, torch.TensorBase.row_indices),
}
    ...
    def test_consistency(self, layout, device, dtype, op):
        ...
        for sample in op.sample_inputs(device, dtype):
            ...
            sample = sample.transform(
                lambda x: (x.clone() if isinstance(x, torch.TensorBase) else x)
            )
...
class TestSparseCSR(TestCase):
    ...
    def test_select(self, device, dtype, index_dtype, layout):
        compressed_indices_mth = {
            torch.sparse_csr: torch.TensorBase.crow_indices,
            torch.sparse_bsr: torch.TensorBase.crow_indices,
            torch.sparse_csc: torch.TensorBase.ccol_indices,
            torch.sparse_bsc: torch.TensorBase.ccol_indices,
        }[layout]

        plain_indices_mth = {
            torch.sparse_csr: torch.TensorBase.col_indices,
            torch.sparse_bsr: torch.TensorBase.col_indices,
            torch.sparse_csc: torch.TensorBase.row_indices,
            torch.sparse_bsc: torch.TensorBase.row_indices,
        }[layout]
        create_tensor_mth = {
            torch.sparse_csr: torch.sparse_csr_tensor,
            torch.sparse_bsr: torch.sparse_bsr_tensor,
        ...
        def make_transposed_addmm_op(f):
            ...
            def wrapper(c, a, b, alpha=None, beta=None, out=None):
                if out is not None:
                    ...
                    assert isinstance(out, torch.TensorBase)
    ...
    def test_sparse_to_sparse_compressed(self, device, dtype, coalesced, layout):
        ...
        if layout in (torch.sparse_bsc, torch.sparse_csc):
            compressed_indices_mth = torch.TensorBase.ccol_indices
            plain_indices_mth = torch.TensorBase.row_indices
        elif layout in (torch.sparse_bsr, torch.sparse_csr):
            compressed_indices_mth = torch.TensorBase.crow_indices
            plain_indices_mth = torch.TensorBase.col_indices
        else:
            raise NotImplementedError("unhandled layout")
...
class TestSparseCompressedTritonKernels(TestCase):
    ...
    def test_triton_kernel(self, op, device, dtype, blocksize):
        ...
        for (
            ...
            kwargs = dict(
                bsr_dense_addmm=dict(beta=beta, alpha=alpha),
                bsr_dense_mm=dict(),
