...
class TestLinalg(TestCase):
    ...
    def test_outer(self, device, dtype):
        def run_test_case(a, b):
            ...
            self.assertEqual(torch.TensorBase.outer(a, b), expected, exact_dtype=False)

            self.assertEqual(torch.ger(a, b), expected, exact_dtype=False)
            self.assertEqual(torch.TensorBase.ger(a, b), expected, exact_dtype=False)
    ...
    def test_addr_type_promotion(self, device):
        for dtypes0, dtypes1, dtypes2 in product(
            ...
            for op in (torch.addr, torch.TensorBase.addr):
                result = op(m, a, b)
    ...
    def test_det(self, device, dtype):
        ...
        ops = (torch.det, torch.TensorBase.det, torch.linalg.det)
        for t in tensors:
    ...
    def _check_einsum(self, *args, np_args=None):
        if np_args is None:
            np_args = [
                arg.cpu().numpy() if isinstance(arg, torch.TensorBase) else arg
                for arg in args
            ]
