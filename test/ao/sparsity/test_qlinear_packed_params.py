...
class TestQlinearPackedParams(TestCase):
    def qlinear_packed_params_test(self, allow_non_zero_zero_points=False):
        ...
        per_channel_small = (
            torch.quantize_per_channel(
                weight_fp32,
                torch.TensorBase(scales),
                torch.TensorBase(zero_points).to(torch.int),
                0,  # axis = 0
                dtype,
            ),
            False,
        ...
        for (
            ...
            # Test Unpacking
            (weights_, bias_, out_features_block_size_, in_features_block_size_) = (
                lin._weight_bias()
            )
    ...
    def test_qlinear_packed_params_fbgemm_qnnpack_cross_compatibility(self):
        ...
        # Test Fbgemm -> Qnnpack
        with override_quantized_engine("fbgemm"):
            packed_params_data_1a, file_buff_1 = (
        ...
        with override_quantized_engine("qnnpack"):
            with override_cpu_allocator_for_qnnpack(qengine_is_qnnpack()):
                packed_params_data_2a, file_buff_2 = (
                    make_lin_get_state_weight_bias_and_save()
                )
