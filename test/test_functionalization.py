...
def _functionalize(
    ...
    def wrapped(*inputs):
        ...
        with ctx:
            inputs_functional = tree_map_only(torch.TensorBase, to_fun, inputs)
            ...
            for inpt, input_functional in zip(flat_inputs, flat_inputs_functional):
                ...
                if inpt_new is not inpt and not skip_input_mutations:
                    ...
                    if inpt_new.shape == inpt.shape:
                        inpt.copy_(inpt_new)
            tree_map_only(torch.TensorBase, torch._sync, out)
            out_unwrapped = tree_map_only(
                torch.TensorBase, torch._from_functional_tensor, out
            )
...
class TestFunctionalization(TestCase):
    ...
    def get_logs(self, func, *inpts, reapply_views=False, run_reinplace=False):
        inpts_clone = tree_map_only(torch.TensorBase, torch.clone, inpts)
        traced_f = make_fx(
    ...
    def assert_functionalization(
        ...
        clones1 = tree_map_only(torch.TensorBase, torch.clone, inpts)
        clones2 = tree_map_only(torch.TensorBase, torch.clone, inpts)
        clones3 = tree_map_only(torch.TensorBase, torch.clone, inpts)
