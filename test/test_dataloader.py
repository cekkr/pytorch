...
class TestDataLoader(TestCase):
    ...
    def test_typing(self):
        ...
        class SomeDatasetClass(Dataset[List[torch.TensorBase]]):
            pass

        def _create_dataloader(is_train: bool) -> DataLoader[List[torch.TensorBase]]:
            pass
        ...
        for num_workers in [0, 1]:
            for chained_dataset in [
                ...
                for e, d in zip(expected, fetched):
                    self.assertIsInstance(d, torch.TensorBase)
...
class TestNamedTupleDataLoader(TestCase):
    ...
    def test_dataloader_with_namedtuple(self):
        ...
        for batch in loader:
            ...
            self.assertIsInstance(batch.data.positive, torch.TensorBase)
        ...
        for batch in loader:
            ...
            self.assertEqual(batch.random_tensor.is_pinned(), TEST_CUDA)
            self.assertIsInstance(batch.data, NamedTupleDataset.Data)
            self.assertNotIsInstance(batch.data.positive, torch.TensorBase)
