...
class TestAsync(JitTestCase):
    ...
    def test_async_future_type_python(self):
        def foo(inp):
            futures = torch.jit.annotate(List[torch.jit.Future[torch.TensorBase]], [])
            for i in range(5):
    ...
    def test_async_parsing(self):
        @torch.jit.script
        def foo(x: TensorBase) -> List[TensorBase]:
            return [torch.neg(x), x.t()]

        @torch.jit.script
        def bar(x):
            futures = torch.jit.annotate(List[Future[List[TensorBase]]], [])
            for _ in range(3):
                future = torch.jit.annotate(
                    Future[List[TensorBase]], torch.jit.fork(foo, x)
                )
                futures.append(future)

            output = torch.jit.annotate(List[List[TensorBase]], [])
            for i in range(3):
    ...
    def test_async_script_trace(self):
        ...
        class Mod(torch.jit.ScriptModule):
            ...
            def forward(
                self, x: TensorBase
            ) -> Tuple[List[TensorBase], Tuple[TensorBase, TensorBase], TensorBase]:
    ...
    def test_trace_fork_wait_list_modulecalls(self):
        ...
        class TestListFutureModule(nn.Module):
            def forward(self, input):
                ...
                fut_list: List[Future[torch.TensorBase]] = []
