...
class TestModuleInterface(JitTestCase):
    ...
    def test_module_interface(self):
        ...
        class OneTwoModule(nn.Module):
            def one(self, x: TensorBase, y: TensorBase) -> TensorBase:
                pass

            def two(self, x: TensorBase) -> TensorBase:
                pass
        ...
        class BarMod(nn.Module):
            ...
            @torch.jit.export
            def forward2(self, x: TensorBase) -> TensorBase:
                return self.two(self.one(x, x)) + 1

        make_global(OneTwoModule, OneTwoClass)

        def use_module_interface(mod_list: List[OneTwoModule], x: torch.TensorBase):
            return mod_list[0].forward(x) + mod_list[1].forward(x)

        def use_class_interface(
            mod_list: List[OneTwoClass], x: TensorBase
        ) -> TensorBase:
        ...
        def call_module_interface_on_other_method(
            mod_interface: OneTwoModule, x: TensorBase
        ) -> TensorBase:
    ...
    def test_module_interface_subtype(self):
        ...
        class TensorToAny(nn.Module):
            def forward(self, input: torch.TensorBase) -> Any:
                pass
        ...
        class TensorToAnyImplB(nn.Module):
            def forward(self, input: Any) -> torch.TensorBase:
    ...
    def test_module_interface_inheritance(self):
        with self.assertRaisesRegex(
            ...
            class InheritMod(nn.ReLU):
                def three(self, x: TensorBase) -> TensorBase:
                    return 3 * x
    ...
    def test_freeze_module_with_interface(self):
        ...
        class ModInterface(torch.nn.Module):
            def forward(self, x: TensorBase) -> int:
                pass
