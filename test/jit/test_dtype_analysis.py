...
class TestDtypeCustomRules(TestDtypeBase):
    ...
    def assert_tensor_dtype_equal(self, tensor_output, graph_dtype):
        if not isinstance(tensor_output, torch.TensorBase):
            return
    ...
    def custom_rules_test_base(self, device, dtype, op, allow_eager_fail=False):
        ...
        input_tensors = [t for t in input_args if isinstance(t, torch.TensorBase)]
        input_tensors += [
            v for v in sample_input.kwargs.values() if isinstance(v, torch.TensorBase)
        ]
