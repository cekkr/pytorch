...
class TestList(JitTestCase):
    ...
    def test_list_sort(self):
        ...
        def test_fail(x: List[TensorBase]) -> List[TensorBase]:
            x.sort()
            return x
    ...
    def test_extend_list_mutable(self):
        @torch.jit.script
        def extend_list(a: List[TensorBase], b: List[TensorBase]) -> List[TensorBase]:
    ...
    def test_copy_list_mutable(self):
        @torch.jit.script
        def copy_list(a: List[TensorBase]) -> List[TensorBase]:
            return a.copy()
    ...
    def test_to_list(self):
        ...
        def to_list_bool_0D(x: torch.TensorBase) -> bool:
            li = torch.jit.annotate(bool, x.tolist())
            return li

        def to_list_bool_1D(x: torch.TensorBase) -> List[bool]:
            li = torch.jit.annotate(List[bool], x.tolist())
            return li

        def to_list_bool_2D(x: torch.TensorBase) -> List[List[bool]]:
            li = torch.jit.annotate(List[List[bool]], x.tolist())
            return li

        def to_list_bool_3D(x: torch.TensorBase) -> List[List[List[bool]]]:
        ...
        def to_list_int_0D(x: torch.TensorBase) -> int:
            li = torch.jit.annotate(int, x.tolist())
            return li

        def to_list_int_1D(x: torch.TensorBase) -> List[int]:
            li = torch.jit.annotate(List[int], x.tolist())
            return li

        def to_list_int_2D(x: torch.TensorBase) -> List[List[int]]:
            li = torch.jit.annotate(List[List[int]], x.tolist())
            return li

        def to_list_int_3D(x: torch.TensorBase) -> List[List[List[int]]]:
        ...
        def to_list_float_0D(x: torch.TensorBase) -> float:
            li = torch.jit.annotate(float, x.tolist())
            return li

        def to_list_float_1D(x: torch.TensorBase) -> List[float]:
            li = torch.jit.annotate(List[float], x.tolist())
            return li

        def to_list_float_2D(x: torch.TensorBase) -> List[List[float]]:
            li = torch.jit.annotate(List[List[float]], x.tolist())
            return li

        def to_list_float_3D(x: torch.TensorBase) -> List[List[List[float]]]:
        ...
        def to_list_complex_0D(x: torch.TensorBase) -> complex:
            li = torch.jit.annotate(complex, x.tolist())
            return li

        def to_list_complex_1D(x: torch.TensorBase) -> List[complex]:
            li = torch.jit.annotate(List[complex], x.tolist())
            return li

        def to_list_complex_2D(x: torch.TensorBase) -> List[List[complex]]:
            li = torch.jit.annotate(List[List[complex]], x.tolist())
            return li

        def to_list_complex_3D(x: torch.TensorBase) -> List[List[List[complex]]]:
        ...
        def to_list_missing_type_annotation(x: torch.TensorBase) -> List[float]:
            li = x.tolist()
            return li

        def to_list_incorrect_type_annotation(x: torch.TensorBase) -> List[float]:
            li = torch.jit.annotate(float, x.tolist())
            return li

        def to_list_unsupported_type_annotation(x: torch.TensorBase) -> List[float]:
            li = torch.jit.annotate(List[str], x.tolist())
            return li

        def to_list_type_annotation_wrong_dim(x: torch.TensorBase) -> List[List[float]]:
            li = torch.jit.annotate(List[List[float]], x.tolist())
            return li
    ...
    def test_no_element_type_annotation(self):
        def fn_with_comment(x: torch.TensorBase) -> List:
            a: List = x.tolist()
            return a

        def annotated_fn(x: torch.TensorBase) -> List:
            a: List = x.tolist()
            return a
...
class TestDict(JitTestCase):
    ...
    def test_keys(self):
        @torch.jit.script
        def keys(x: Dict[str, TensorBase]) -> List[str]:
            return list(x.keys())
    ...
    def test_values(self):
        @torch.jit.script
        def values(x: Dict[str, TensorBase]) -> List[TensorBase]:
    ...
    @skipIfTorchDynamo("TorchDynamo fails for this test for unknown reason")
    def test_len(self):
        def length(x: Dict[str, TensorBase]) -> int:
            return len(x)
    ...
    @skipIfTorchDynamo("TorchDynamo fails for this test for unknown reason")
    def test_copy(self):
        def func(x: Dict[str, TensorBase]) -> Dict[str, TensorBase]:
            return x.copy()
    ...
    @skipIfTorchDynamo("TorchDynamo fails for this test for unknown reason")
    def test_items(self):
        def func(x: Dict[str, TensorBase]) -> List[Tuple[str, TensorBase]]:
            return x.items()
    ...
    def test_pop(self):
        def pop(
            x: Dict[str, TensorBase], key: str
        ) -> Tuple[TensorBase, Dict[str, TensorBase]]:
            return x.pop(key), x
        ...
        def default_pop(
            x: Dict[str, TensorBase], key: str, default: TensorBase
        ) -> Tuple[TensorBase, Dict[str, TensorBase]]:
    ...
    def test_update(self):
        def update(
            a: Dict[str, TensorBase], b: Dict[str, TensorBase]
        ) -> Tuple[Dict[str, TensorBase], Dict[str, TensorBase]]:
            a.update(b)
            return a, b
    ...
    def test_aug_assign(self):
        def aug_assign_dict_tensor(a: Dict[str, TensorBase]) -> Dict[str, TensorBase]:
            a["a"] += 1
            a["b"] -= 12
            a["c"] *= 122
            a["c"] /= 2
    ...
    def test_popitem(self):
        @torch.jit.script
        def popitem(
            x: Dict[str, TensorBase]
        ) -> Tuple[Tuple[str, TensorBase], Dict[str, TensorBase]]:
            item = x.popitem()
            return item, x
        ...
        self.assertTrue(isinstance(script_out[0][1], torch.TensorBase))

    @skipIfTorchDynamo("TorchDynamo fails for this test for unknown reason")
    def test_clear(self):
        def clear(x: Dict[str, TensorBase]) -> Dict[str, TensorBase]:
            x.clear()
            return x
    ...
    @skipIfTorchDynamo("TorchDynamo fails for this test for unknown reason")
    def test_get(self):
        def get(x: Dict[str, TensorBase], key: str) -> Optional[TensorBase]:
            return x.get(key)

        self.checkScript(get, (self.dict(), "a"))
        self.checkScript(get, (self.dict(), "doesn't exist"))

        def get_default(x: Dict[str, TensorBase], key: str) -> Optional[TensorBase]:
    ...
    def test_basic(self):
        ...
        def type_default() -> Dict[str, TensorBase]:
            return {}
        ...
        def list_of_dicts() -> List[Dict[str, TensorBase]]:
    ...
    def test_optional_dict_construct(self):
        class M(torch.nn.Module):
            def use(self, buffer: Dict[str, Optional[torch.TensorBase]]):
                return buffer["prev_key"]

            def forward(self, x):
                prev_key = torch.rand(2, 3)
                next_key = torch.rand(2, 3)
                saved_state: Dict[str, Optional[torch.TensorBase]] = {
                    "prev_key": prev_key,
                    "next_key": next_key,
                }
