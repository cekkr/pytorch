...
class TestSymbolicShapeAnalysis(JitTestCase):
    ...
    def test_unary_shape_fns_inplace(self):
        def mul_inplace(x: torch.TensorBase):
            y = x.mul_(2)
            return y
    ...
    def test_binary_shape_fns_inplace(self):
        def div_inplace_tensor(x: torch.TensorBase, y: torch.TensorBase):
            z = x.div_(y)
            return z

        def add_inplace_tensor(x: torch.TensorBase, y: torch.TensorBase):
            z = x.add_(y)
            return z
