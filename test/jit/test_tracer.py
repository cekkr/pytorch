...
class TestTracer(JitTestCase):
    ...
    def test_trace_checking_with_global_name(self):
        class MyClass(torch.nn.Module):
            def forward(self, xs: List[TensorBase]):
    ...
    def test_trace_inline_shape(self):
        ...
        @torch.jit.script
        def tensor_size(x: torch.TensorBase) -> torch.TensorBase:
    ...
    def test_trace_optional(self):
        @torch.jit.script
        def test(x: Optional[TensorBase]):
            if x is None:
    ...
    def test_trace_func_argument_names_captured(self):
        def fn(
            first_arg: torch.TensorBase, second_arg: torch.TensorBase
        ) -> torch.TensorBase:
    ...
    def test_trace_partial_func_argument_names_captured(self):
        def fn(first_arg: torch.TensorBase, second_arg=1) -> torch.TensorBase:
            return first_arg + second_arg
    ...
    def test_trace_module_argument_names_captured(self):
        class TestModule(nn.Module):
            ...
            def forward(
                self, first_arg: torch.TensorBase, second_arg: torch.TensorBase
            ):
...
class TestMixTracingScripting(JitTestCase):
    def test_trace_script(self):
        @torch.jit.script
        def func1(x: Tuple[TensorBase, TensorBase]) -> TensorBase:
            return x[0] + x[1]

        @torch.jit.script
        def func2(x: List[TensorBase]) -> TensorBase:
            return x[0] + x[1]
        ...
        @torch.jit.script
        def func3(
            x: TensorBase, method: str = "bilinear", align_corners: bool = True
        ) -> TensorBase:
            hw = x.shape[2:4]
        ...
        @torch.jit.script
        def func4(x: TensorBase, a: List[Optional[str]]) -> TensorBase:
            if len(a) == 2:
                return x + 2
            else:
                return x
    ...
    def test_trace_mixed_by_script_with_dict_output(self):
        @torch.jit.script
        def return_dict(input: torch.TensorBase) -> Dict[str, torch.TensorBase]:
            return {"foo": input + 1}
    ...
    def test_trace_dict_mix_script(self):
        class testB(torch.nn.Module):
            ...
            def forward(self, feature_map: Dict[str, List[TensorBase]]) -> TensorBase:
                output = []
        ...
        class testA(torch.nn.Module):
            ...
            def forward(self, input_map: Dict[str, List[TensorBase]]) -> TensorBase:
                feature_map = {}
    ...
    def test_trace_script_returning_complex_dict(self):
        ...
        class ReturnsDict(torch.nn.Module):
            def forward(
                self,
                id_score_list: Dict[
                    str, Tuple[torch.TensorBase, torch.TensorBase, torch.TensorBase]
                ],
            ) -> Dict[str, Tuple[torch.TensorBase, torch.TensorBase, torch.TensorBase]]:
    ...
    def test_trace_returning_dict_with_tensor_tuples(self):
        ...
        class ReturnsDict(torch.nn.Module):
            def forward(
                self, k: torch.TensorBase, v: torch.TensorBase
            ) -> Dict[str, Tuple[torch.TensorBase, torch.TensorBase]]:
                x = 2 * k
                y = 3 * v
                result = {"imakey": (x, y)}
                return result
        ...
        class ReturnsBadDict(torch.nn.Module):
            def forward(
                self, k: torch.TensorBase, v: torch.TensorBase
            ) -> Dict[str, Tuple[torch.TensorBase, float]]:
                x = 2 * k
                result = {"imakey": (x, 1)}
                return result
