...
class TestNestedTensor(TestCase):
    ...
    def test_2d_nested_tensor(self, batch_size, max_seq_len, vocab_size):
        ...
        for _ in range(batch_size):
            ...
            data.append(row)
            nested_tensor_ref_list.append(torch.TensorBase(row))
...
class TestNestedTensorSubclass(TestCase):
    ...
    def test_binary_pointwise_transposed(self, device):
        ...
        self.assertEqual(
            out.is_contiguous(),
            (b.transpose(-1, -2) * b.transpose(-1, -2)).is_contiguous(),
        )
    ...
    def test_jagged_layout_construction_nested_tensor(
        ...
        for tensor_list in self._get_example_tensor_lists(
            ...
            for t in tensor_list:
                t = t if isinstance(t, torch.TensorBase) else torch.as_tensor(t)
    ...
    def test_jagged_layout_construction_with_pinned_memory(self, device):
        for tensor_list in self._get_example_tensor_lists():
            ...
            self._validate_nt(
                nt, tensor_list, "cpu", torch.float32, requires_grad=False
            )
    ...
    def test_sdpa(self, device, dtype):
        ...
        def check_forward_backward():
            ...
            nt_grads = torch.autograd.grad(
                buffer_from_jagged(attn_nt).sum(), (q_nt, k_nt, v_nt)
            )
