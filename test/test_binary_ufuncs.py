...
class TestBinaryUfuncs(TestCase):
    ...
    def _test_pow(self, base, exponent, np_exponent=None):
        ...
        try:
            ...
            if isinstance(base, torch.TensorBase):
                ...
                if (
                    isinstance(exponent, torch.TensorBase)
                    and base.dim() == 0
            ...
            for a, b in product(range(-10, 10), range(-10, 10)):
                for op in (lambda x: x * 0.5, lambda x: math.floor(x)):
                    ...
                    for args in product(vals, vals):
                        ...
                        first_scalar = (
                            first
                            if not isinstance(first, torch.TensorBase)
                            else first.item()
                        )
                        second_scalar = (
                            second
                            if not isinstance(second, torch.TensorBase)
                            else second.item()
                        )
    ...
    def test_div(self, device, dtype):
        for op, method, inplace in (
            (torch.div, torch.TensorBase.div, torch.TensorBase.div_),
            (
                torch.true_divide,
                torch.TensorBase.true_divide,
                torch.TensorBase.true_divide_,
            ),
        ):
    ...
    def test_fmod_remainder(self, device, dtype):
        ...
        for dividend, mod in product([x, x.t()], mods):
            _helper(
                ...
                mod,
                (
                    (torch.fmod, torch.TensorBase.fmod_, np.fmod),
                    (torch.remainder, torch.TensorBase.remainder_, np.remainder),
                ),
            )
    ...
    def test_lerp_lowp(self, device, dtype):
        ...
        for x, y, w in zip(xs, ys, weights):
            xref = x.float()
            yref = y.float()
            wref = w.float() if isinstance(w, torch.TensorBase) else w
    ...
    @skipIf(sys.version_info >= (3, 12), "Failing on Python 3.12")
    ...
    def test_float_power(self, device, dtypes):
        def to_np(value):
            if isinstance(value, torch.TensorBase) and value.dtype == torch.bfloat16:
                return value.to(torch.float).cpu().numpy()
            return value.cpu().numpy() if isinstance(value, torch.TensorBase) else value
        ...
        for op in (
            ...
            torch.TensorBase.float_power_,
        ):

            # Case of Tensor x Tensor
            if op is torch.TensorBase.float_power_ and base_dtype != out_dtype:
                with self.assertRaisesRegex(
            ...
            for i in complex_exponents if exp_dtype.is_complex else exponents:
                ...
                if (
                    op is torch.TensorBase.float_power_
    ...
    def test_float_power_exceptions(self, device):
        def _promo_helper(x, y):
            for i in (x, y):
                ...
                elif type(i) == torch.TensorBase and i.is_complex():
                    return torch.complex128
            return torch.double
        ...
        for base, exp in test_cases:
            for out_dtype in (torch.long, torch.float, torch.double, torch.cdouble):
                ...
                if base.dtype == required_dtype:
                    torch.TensorBase.float_power_(base.clone(), exp)
                else:
                    with self.assertRaisesRegex(
                        RuntimeError, "operation's result requires dtype"
                    ):
