...
class TestTEFuser(JitTestCase):
    ...
    def test_scalar_arg(self):
        for device in self.devices:
            ...
            def fn_test_scalar_arg(x: torch.TensorBase, p: float) -> torch.TensorBase:
                return p * (x * x + x)
    ...
    def test_neg_pow(self):
        def eager_tt(a: torch.TensorBase, b: torch.TensorBase):
            return torch.neg(torch.pow(a, b))

        def eager_ts(a: torch.TensorBase, b: float):
            return torch.neg(torch.pow(a, b))

        def eager_st(a: float, b: torch.TensorBase):
            return torch.neg(torch.pow(a, b))
    ...
    def test_dynamic_cat(self):
        with inline_fusion_groups():
            ...
            @torch.jit.script
            def repro(
                xs: List[torch.TensorBase],
                ys: List[torch.TensorBase],
                zs: List[torch.TensorBase],
            ):
                return [
    ...
    def test_pow_multiple_dtype(self):
        ...
        def fn(p: torch.TensorBase, gamma: float = 2.0) -> torch.TensorBase:
            p = torch.sigmoid(p)
            result = p**gamma
            return result
