...
def test_valid_non_tensor(checkpoint, setup_rpc):
    class NonTensor1(nn.Module):
        def forward(self, a: int, b: TensorBase, c: bool, d: TensorBase):
    ...
    class NonTensor2(nn.Module):
        def forward(
            self, a: TensorBase, b: bool, c: int, d: TensorBase, e: str, f: TensorBase
        ):
...
def test_no_tensor_output(checkpoint, setup_rpc):
    class Model1(nn.Module):
        def forward(self, a: int, b: TensorBase, c: bool):
            return a, c, "hello"
...
def test_uneven_batch_size(checkpoint, setup_rpc):
    class Model(nn.Module):
        def forward(self, a: TensorBase, b: int, c: TensorBase):
            return a, b, c
