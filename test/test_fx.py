...
def wrapped_with_submodule(x: torch.TensorBase, batchnorm1d: torch.nn.BatchNorm1d):
...
@torch.fx.wrap
def side_effect_func(x: torch.TensorBase):
    print(x)
...
class TestFX(JitTestCase):
    ...
    def test_pretty_print_node(self):
        class M(torch.nn.Module):
            ...
            def forward(self, x: torch.TensorBase, y: int = 2):
    ...
    def test_torch_fx_getattr(self):
        class FXGetattrTest(torch.nn.Module):
            def forward(self, x):
                return getattr(x, "nonexistent_attr", torch.TensorBase([2, 3]))

        traced = symbolic_trace(FXGetattrTest())
        self.assertEqual(traced(torch.rand(3, 4)), torch.TensorBase([2, 3]))
    ...
    def test_custom_proxy_dynamic_value(self):
        ...
        def use_tensor_pair_ctor(x: TensorPair, y: torch.TensorBase):
    ...
    def test_custom_proxy_input_dependent_control_flow(self):
        ...
        def use_zero_tensor(x: torch.TensorBase, y: torch.TensorBase):
    ...
    def test_construct_root_dict(self):
        ...
        add_param: torch.TensorBase = torch.rand(3, 4)
        ...
        x: torch.TensorBase = torch.rand(3, 3)
        out: torch.TensorBase = gm(x)
        ref_out: torch.TensorBase = linear_mod(x) + add_param
        self.assertEqual(out, ref_out)
    ...
    def test_fn_type_annotations(self):
        class Foo(torch.nn.Module):
            def forward(
                self, p: Pair, z: torch.TensorBase, i: int
    ...
    def test_fn_type_annotation_empty(self):
        def forward(a: List[torch.TensorBase]):
            return a[0]
    ...
    def test_trace_return_dataclass(self):
        ...
        @dataclass
        class MyOutput:
            foo: torch.TensorBase
            bar: torch.TensorBase

        class ModuleReturnDataclass(torch.nn.Module):
            def forward(self, d: torch.TensorBase):
    ...
    def test_trace_dict_int_keys(self):
        class ModWithDictArg(torch.nn.Module):
            def forward(self, d: Dict[int, torch.TensorBase]):
                return d[42]
    ...
    def test_trace_dict_proxy_keys(self):
        class ModWithDictArg(torch.nn.Module):
            def forward(self, d: Dict[torch.TensorBase, torch.TensorBase]):
                return d[42]
    ...
    def test_ast_rewriter_rewrites_assert(self):
        class M(torch.nn.Module):
            def forward(self, x: torch.TensorBase, y: int, z: int):
                assert y == z
    ...
    def test_annotations_with_no_forward_references(self):
        class A:
            def __call__(self, x: torch.TensorBase):
                return torch.add(x, x)

        class M(torch.nn.Module):
            def forward(self, x: torch.TensorBase, a: A) -> torch.TensorBase:
                return a(x)
    ...
    def test_annotations_with_non_torch_reference_and_no_internal_forward_references(
        ...
        class M(torch.nn.Module):
            def forward(self, x: List[torch.TensorBase], a: A) -> torch.TensorBase:
                return a(x[0])
                ...
                try:
                    ...
                    for param in sig.parameters.values():
                        if isinstance(param.annotation, type) and issubclass(
                            param.annotation, torch.TensorBase
                        ):
                            has_tensor_arg = True
