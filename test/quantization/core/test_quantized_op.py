    ...
    def test_custom_module_multi_head_attention(self):
        class MultiheadAttentionModel(torch.nn.Module):
            ...
            def forward(
                ...
                key,
                value,
                key_padding_mask: Optional[torch.TensorBase] = None,
...
class TestQuantizedLinear(TestCase):
    ...
    def _test_qlinear_pt2e_helper(
        self, post_op_to_qlinear_ref_dict, supported_post_ops, post_op_algorithms
    ):
        ...
        with override_quantized_engine("onednn"):
            for (
                ...
                if weight_quant_per_channel:
                    w_scales = torch.TensorBase([w_scale] * oc)
                    w_zps = torch.zeros(oc).to(dtype=torch.int)
                    qw = torch.quantize_per_channel(w, w_scales, w_zps, 0, torch.qint8)
                else:
                    w_scales = torch.TensorBase([w_scale])
                    w_zps = torch.TensorBase([w_zp]).to(dtype=torch.int)
                ...
                if post_op == "gelu":
                    qlinear_ref = torch.ops.quantized.linear(
                        qx, qw_packed_ref, used_y_scale, used_y_zp
                    )
                    qy_ref = torch.nn.functional.gelu(
                        qlinear_ref, approximate=post_op_algo
                    )
                else:
    ...
    def test_qlinear_linear_relu_pt2e(self):
        ...
        self._test_qlinear_pt2e_helper(
            post_op_to_qlinear_ref_dict, supported_post_ops, post_op_algorithms
        )
    ...
    def _test_qconv_impl_cpu_tensor(
        ...
        if post_op.binary_attr == "sum":
            ...
            Y_q_cpu_tensor = qconv(
                ...
                groups,
                1.0
                / Y_scale,  # Kernel expects pass in reciprocal of scale in fake quant
                Y_zero_point,
                qconv_output_dtype,
                post_op.binary_attr,
                post_op.alpha,
