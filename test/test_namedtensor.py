...
class TestNamedTensor(TestCase):
    ...
    def _test_name_inference(
        ...
        casted_args = [
            arg.to(device) if isinstance(arg, torch.TensorBase) else arg for arg in args
        ]
    ...
    def test_detach(self):
        names = ["N"]
        self._test_name_inference(
            TensorBase.detach_, [torch.randn(3, requires_grad=True, names=names)], names
        )
        self._test_name_inference(
            TensorBase.detach, [torch.randn(3, requires_grad=True, names=names)], names
        )
