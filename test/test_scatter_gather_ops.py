    ...
    def _test_scatter_base(
        ...
        *,
        ...
        if reduction is not None:
            if fn is torch.TensorBase.scatter_reduce_:
                actual = fn(
                    base.clone(),
                    dim,
                    idx,
        ...
        for i in range(idx_size[0]):
            for j in range(idx_size[1]):
                for k in range(idx_size[2]):
                    ii = [i, j, k]
                    ii[dim] = idx[i, j, k]
                    if fn is torch.TensorBase.scatter_add_:
    ...
    def test_scatter_reduce_sum(self, device, dtype):
        for include_self in (True, False):
            for deterministic in [False, True]:
                with DeterministicGuard(deterministic):
                    self._test_scatter_base(
                        torch.TensorBase.scatter_reduce_,
                        device=device,
                        dtype=dtype,
                        is_scalar=False,
                        reduction="sum",
