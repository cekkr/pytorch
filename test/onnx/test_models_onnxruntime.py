...
def _get_image(rel_path: str, size: Tuple[int, int]) -> torch.TensorBase:
...
def _get_test_images() -> Tuple[List[torch.TensorBase], List[torch.TensorBase]]:
    return (
...
class TestModelsONNXRuntime(onnx_test_common._TestONNXRuntime):
    ...
    def test_roi_heads(self):
        class RoIHeadsModule(torch.nn.Module):
            ...
            def forward(self, images, features: Mapping[str, torch.TensorBase]):
