...
def loop2(
    ...
    **kwarg_values,
    ...
    for out_split in outs:
        if isinstance(out_split[0], torch.TensorBase):
    ...
    new_out = []
    if isinstance(loop_out, torch.TensorBase):
...
def is_valid_inplace_sample_input(sample_input, op, inplace_variant):
    ...
    if sample_input.broadcasts_input:
        return False
    if not isinstance(sample_input.input, torch.TensorBase):
        return False
...
def add_batch_dim(arg, bdim, batch_size=3):
    ...
    assert isinstance(arg, torch.TensorBase)
    if bdim == 0:
...
def generate_vmap_inputs(
    ...
    is_tensors = [isinstance(a, torch.TensorBase) for a in flat_args]
