...
def is_in_base(t, maybe_tensors):
    ...
    for maybe_tensor in maybe_tensors:
        if isinstance(maybe_tensor, torch.TensorBase):
    ...
    def verify_aot_autograd(
        ...
        *,
        ...
        for keep_input_mutations in (
            ...
            if isinstance(ref_out, torch.TensorBase):
                self.assertTrue(isinstance(test_out, torch.TensorBase))
                ref_out, test_out = [ref_out], [test_out]
            for ref_o, test_o in zip(ref_out, test_out):
                if isinstance(ref_o, torch.TensorBase):
            ...
            for ref_i, test_i in zip(inp, inp_copy):
                if isinstance(ref_i, torch.TensorBase):
...
def _test_aot_autograd_module_helper(
    ...
    for module_input in module_inputs:
        ...
        sentinel_val = -42
        is_tensor_spec = [
            sentinel_val if isinstance(arg, torch.TensorBase) else arg
            for arg in flat_args
        ]
        args = [arg for arg in flat_args if isinstance(arg, torch.TensorBase)]
