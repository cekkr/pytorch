...
def reference_vmap(op, inputs, in_dims=0, out_dims=0, return_nt=False):
    ...
    if op_has_single_return:
        assert all(isinstance(result, torch.TensorBase) for result in results)
...
class TestVmapOperators(Namespace.TestVmapBase):
    ...
    def test_unary_pointwise(self, case):
        ...
        # test in-place
        method = getattr(TensorBase, f'{op.__name__ + "_"}')
    ...
    def test_copy_(self):
        x = torch.randn(3)
        y = torch.randn(3)
        vmap(TensorBase.copy_)(x, y)
        self.assertEqual(x, y)

        x = torch.randn(3)
        y = torch.randn(3, 2)
        vmap(TensorBase.copy_, in_dims=(1, None))(y, x)
        self.assertEqual(y, x.expand(2, 3).t())

        x = torch.randn(3)
        y = torch.randn(2, 3)
        with self.assertRaisesRegex(RuntimeError, "inplace"):
            vmap(TensorBase.copy_, in_dims=(None, 0))(x, y)
    ...
    def test_contiguous(self):
        ...
        x = x.movedim(0, 2)
        result = vmap(TensorBase.contiguous, in_dims=2, out_dims=2)(x)
        self.assertTrue(result is x)
    ...
    def test_fill_and_zero_inplace(self):
        ...
        B0, B1 = 3, 5
        test(TensorBase.fill_, [TensorFactory.randn([B0, B1]), TensorFactory.randn(B0)])

        with self.assertRaisesRegex(RuntimeError, ""):
            # Runtime Error is thrown when the tensor being written to isn't being vmapped over
            vmap(TensorBase.fill_, (None, 0))(
    ...
    def test_reshape_as(self):
        test = self._vmap_test
        B0, B1, B2 = 7, 11, 13
        op = torch.TensorBase.reshape_as
...
class TestVmapBatchedGradient(Namespace.TestVmapBase):
    ...
    def test_lgamma(self, device):
        x = torch.randn(2, 3, requires_grad=True, device=device)
        self._batched_grad_test(TensorBase.lgamma, (x,))
        self._batched_grad_grad_test(TensorBase.lgamma, (x,))
    ...
    def test_sigmoid(self, device):
        x = torch.randn(2, 3, requires_grad=True, device=device)
        self._batched_grad_test(TensorBase.sigmoid, (x,))
        self._batched_grad_grad_test(TensorBase.sigmoid, (x,))
    ...
    def test_trace(self, device):
        x = torch.randn(2, 3, device=device, requires_grad=True)
        self._batched_grad_test(TensorBase.trace, (x,))
...
class TestVmapOperatorsOpInfo(TestCase):
    ...
    def opinfo_vmap_test(
        ...
        def test():
            ...
            for sample_input in sample_inputs_itr:
                ...
                if not any(isinstance(arg, torch.TensorBase) for arg in args):
    ...
    def test_fill__Tensor(self, device):
        ...
        def test():
            ...
            self.vmap_inplace_test(TensorBase.fill_, args, {}, (0, 0))

            args = (torch.randn(3, B, device=device), torch.randn(B))
            self.vmap_inplace_test(TensorBase.fill_, args, {}, (-1, 0))

            args = (torch.randn(3, device=device), torch.randn(B))
            self.vmap_inplace_test(TensorBase.fill_, args, {}, (None, 0))

            args = (torch.randn(3, B, device=device), torch.randn([]))
            self.vmap_inplace_test(TensorBase.fill_, args, {}, (1, None))

        check_vmap_fallback(self, test, TensorBase.fill_)
