...
def fork_wait_graph3(input, iters: int):
    futures: List[torch.jit.Future[torch.TensorBase]] = []
...
class TestStaticModule(TestCase):
    ...
    def test_create_object(self):
        class Foo:  # noqa: B903
            def __init__(self, x: torch.TensorBase) -> None:
                self.x = x
        ...
        class Mod(torch.nn.Module):
            ...
            def forward(self, y: torch.TensorBase) -> torch.TensorBase:
                foo = Foo(y)
                return y * foo.x
