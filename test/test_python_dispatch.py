...
$1: f32[] = torch._ops.my_lib.weird.default(['None', '$0'])""",
        ...
        for list_type in (list, tuple):
            ...
            class A(torch._C.TensorBase):
                ...
                def __new__(cls, elem):
                    return torch.TensorBase._make_subclass(
    ...
    def test_invalid_ret(self) -> None:
        ...
        class A(torch._C.TensorBase):
            @staticmethod
            def __new__(cls, elem):
                return torch.TensorBase._make_subclass(cls, elem, elem.requires_grad)
...
$2: f32[1] = torch._ops.aten.detach.default($1)""",
    ...
    def test_make_wrapper_subclass_noalloc(self) -> None:
        ...
        torch.TensorBase._make_wrapper_subclass(LoggingTensor, (1000000000000,))
...
$6: f32[1] = torch._ops.aten.add_.Tensor($1, $5)""",
    ...
    def test_subclass_creation(self):
        ...
        with self.assertRaisesRegex(TypeError, "Foo must define __torch_dispatch__"):
            torch.TensorBase._make_wrapper_subclass(Foo, (2, 2))

    def test_new_ones(self) -> None:
        class MyTensor(torch.TensorBase):
            @classmethod
            def __torch_dispatch__(cls, func, types, args=(), kwargs=None):
                return MyTensor(3)
...
def forward(self, x_a_1, x_b_1, y_1):
    ...
    def test_make_wrapper_subclass_propagates_metadata(self) -> None:
        class WrapperTensor(torch.TensorBase):
    ...
    def test_deepcopy_wrapper_subclass_with_clone_returning_different_type(
        ...
        class MyWrapperTensor(torch.TensorBase):
            elem: torch.TensorBase
    ...
    def test_deepcopy_non_wrapper_subclass(self) -> None:
        ...
        class SubTensorSuccess(torch.TensorBase):
            def new_empty(self, shape):
    ...
    def test_wrapper_subclass_extra_dispatch_keys(self) -> None:
        class ExtraKeysTensor(torch.TensorBase):
            @staticmethod
            def __new__(cls, elem, *args, **kwargs):
    ...
    def test_index_put_where_only_index_is_subclass(self) -> None:
        ...
        class MyTensor(torch.TensorBase):
            ...
            @staticmethod
            def __new__(cls, elem, *args, **kwargs):
                r = torch.TensorBase._make_wrapper_subclass(
                    cls,
                    elem.size(),
                    dtype=elem.dtype,
                    layout=elem.layout,
...
$3: f32[] = torch._ops.aten.add.Tensor($1, $2)""",
    ...
    def test_shallow_copy_and_detach(self) -> None:
        ...
        class TestMode(TorchDispatchMode):
            def __torch_dispatch__(self, func, types, args=(), kwargs=None):
                tree_map_only(
                    torch.TensorBase,
                    lambda t: test_case.assertIn(t, seen),
                    (args, kwargs),
                )
                if kwargs is None:
                    kwargs = {}
                r = func(*args, **kwargs)
                tree_map_only(torch.TensorBase, lambda t: seen.add(t), r)
                return r
    ...
    def test_make_subclass_with_modes(self):
        class ModeTensor(torch.TensorBase):
            def __new__(cls, elem, mode):
                r = torch.TensorBase._make_subclass(cls, elem, elem.requires_grad)
                r.elem = elem
                r.mode = mode
                return r
    ...
    def test_notimplemented_mode(self):
        ...
        class SubTensor(torch.TensorBase):
            def __new__(cls, elem):
                r = torch.TensorBase._make_wrapper_subclass(cls, elem.shape)
                r.elem = elem
                return r
...
$0: f32[] = torch._ops.aten.empty.memory_format([], device=device(type='cpu'), pin_memory=False)""",
    ...
    def test_record_stream(self) -> None:
        class TestMode(TorchDispatchMode):
            ...
            def __torch_dispatch__(self, func, types, args=(), kwargs=None):
                ...
                self.testcase.assertIsInstance(args[0], torch.TensorBase)
    ...
    def test_subclass_autograd_device_check(self) -> None:
        class NonWrapperSubclass(torch.TensorBase):
            elem: torch.TensorBase
            ...
            def __new__(cls, elem, *args, **kwargs):
                # Wrong device here!
                r = torch.TensorBase._make_subclass(
            ...
            def __torch_dispatch__(cls, func, types, args=(), kwargs=None):
                ...
                def wrap(e):
                    return (
                        NonWrapperSubclass(e) if isinstance(e, torch.TensorBase) else e
                    )
    ...
    def test_none_wrapping(self):
        ...
        class SubclassWithNone(torch.TensorBase):
            @staticmethod
            def __new__(cls, elem, *args, **kwargs):
                r = torch.TensorBase._make_wrapper_subclass(
                    cls,
            ...
            def __torch_dispatch__(cls, func, types, args=(), kwargs=None):
                ...
                def wrap(e):
                    return SubclassWithNone(e) if isinstance(e, torch.TensorBase) else e
    ...
    def test_dispatch_super_call(self):
        ...
        class SubTensor(torch.TensorBase):
            @staticmethod
            def __new__(cls, elem):
                return torch.TensorBase._make_subclass(cls, elem)
    ...
    def test_dispatch_super_call_list_arg(self):
        ...
        class SubTensorWithListArg(torch.TensorBase):
            @staticmethod
            def __new__(cls, elem):
                return torch.TensorBase._make_subclass(cls, elem)
    ...
    def test_multiple_ops_subclass(self):
        ...
        class MySubclass(torch.TensorBase):
            @staticmethod
            def __new__(cls, elem):
                r = torch.TensorBase._make_subclass(cls, elem)
                return r
    ...
    def subclass_helper(cls, data, use_wrapper_subclass, **kwargs):
        if use_wrapper_subclass:
            ...
            return torch.TensorBase._make_wrapper_subclass(cls, data.size(), **kwargs)  # type: ignore[attr-defined]
        else:
            return torch.TensorBase._make_subclass(cls, data, True, **kwargs)
    ...
    def test_is_contiguous_slow_path(self):
        ...
        for use_wrapper_subclass in [True, False]:
            ...
            class ExampleTensor1(torch.TensorBase):
                @staticmethod
            ...
            class ExampleTensor2(torch.TensorBase):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="strides"
            ...
            class ExampleTensor3(torch.TensorBase):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="strides"
    ...
    def test_fancy_strides(self):
        ...
        class ExampleTensor(torch.TensorBase):
            @staticmethod
    ...
    def test_dim_slowpath(self):
        ...
        for use_wrapper_subclass in [True, False]:
            ...
            class DimNotImplementedTensor(torch.TensorBase):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
            ...
            class DimImplementedTensor(torch.TensorBase):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="sizes"
    ...
    def test_sym_sizes_strides_slow_path(self):
        class TestTensor(torch.TensorBase):
            @staticmethod
    ...
    def test_strides_slow_path(self):
        for use_wrapper_subclass in [True, False]:
            ...
            class StridesNotImplemented(torch.TensorBase):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="strides"
            ...
            class StridesCustomReturn(torch.TensorBase):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="strides"
            ...
            class StridesDefaultReturn(torch.TensorBase):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="strides"
    ...
    def test_sizes_slow_path(self):
        for use_wrapper_subclass in [True, False]:
            ...
            class SizesNotImplemented(torch.TensorBase):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="sizes"
            ...
            class SizesCustomReturn(torch.TensorBase):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="sizes"
            ...
            class SizesDefaultReturn(torch.TensorBase):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_sizes_strides_policy="sizes"
    ...
    def test_custom_size_policy_dynamic_shapes(self):
        ...
        class CustomSizeDynamicShapesTensor(torch.TensorBase):
            @staticmethod
...
def forward(self, x_1):
    ...
    def test_data_ptr_respects_numel_slow_path(self):
        ...
        class NumelDefaultReturn(torch.TensorBase):
            @staticmethod
            def __new__(cls, data, wrapper):
                return TestPythonDispatch.subclass_helper(
                    cls, data, wrapper, dispatch_sizes_strides_policy="sizes"
    ...
    def test_layout_slow_path(self):
        for use_wrapper_subclass in [True, False]:
            ...
            class LayoutNotImplemented(torch.TensorBase):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
            ...
            class LayoutCustomReturn(torch.TensorBase):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_layout=True
            ...
            class LayoutDefaultReturn(torch.TensorBase):
                @staticmethod
                def __new__(cls, data, wrapper):
                    return TestPythonDispatch.subclass_helper(
                        cls, data, wrapper, dispatch_layout=True
...
class TestWrapperSubclassAliasing(TestCase):
    ...
    def _test_wrapper_subclass_aliasing(self, op, args, kwargs):
        def to_subclass(t: torch.TensorBase):
            return TwoTensor(t, t.clone())

        result_ref = op(*args, **kwargs)

        args_subclass = pytree.tree_map_only(torch.TensorBase, to_subclass, args)
        kwargs_subclass = pytree.tree_map_only(torch.TensorBase, to_subclass, kwargs)

        result_test = op(*args_subclass, **kwargs_subclass)

        args_ref_flat = pytree.arg_tree_leaves(*args, **kwargs)
        args_ref_flat_tensors = [
            x for x in args_ref_flat if isinstance(x, torch.TensorBase)
        ]

        args_test_flat = pytree.tree_leaves((args_subclass, kwargs_subclass))
        args_test_flat_tensors = [
            x for x in args_test_flat if isinstance(x, torch.TensorBase)
        ]

        result_ref_flat = pytree.tree_leaves(result_ref)
        result_ref_flat_tensors = [
            x for x in result_ref_flat if isinstance(x, torch.TensorBase)
        ]

        result_test_flat = pytree.tree_leaves(result_test)
        result_test_flat_tensors = [
            x for x in result_test_flat if isinstance(x, torch.TensorBase)
        ]
