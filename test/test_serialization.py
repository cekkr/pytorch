...
class SerializationMixin:
    ...
    def test_serialization_sparse_csr_invalid(self):
        self._test_serialization_sparse_compressed_invalid(
            torch.TensorBase.to_sparse_csr,
    ...
    def test_serialization_sparse_csc_invalid(self):
        self._test_serialization_sparse_compressed_invalid(
            torch.TensorBase.to_sparse_csc,
    ...
    def _test_serialization_backwards_compat(self, weights_only):
        ...
        class OldTensorV1(OldTensorBase):
            def __reduce__(self):
                return (torch.TensorBase, (), self.__getstate__())
...
class TestSerialization(TestCase, SerializationMixin):
    ...
    @unittest.skipIf(
        not torch.cuda.is_available() or IS_WINDOWS,
    ...
    def test_serialization_dtype(self, dtype, weights_only):
        ...
        with tempfile.NamedTemporaryFile() as f:
            ...
            self.assertEqual(
                y["x"][:2].to(dtype=torch.float32), torch.tensor([-0.25, 0.25])
            )
...
class TestWrapperSubclass(torch.TensorBase):
    ...
    def __new__(cls, elem, *args, **kwargs):
        ...
        r = torch.TensorBase._make_subclass(cls, elem.to("meta"), elem.requires_grad)
...
class TestGetStateSubclass(torch.TensorBase):
    elem: torch.TensorBase
    __slots__ = ["elem"]
...
class TestEmptySubclass(torch.TensorBase): ...
