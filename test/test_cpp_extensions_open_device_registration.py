...
class DummyModule:
    ...
    @staticmethod
    def get_rng_state(
        device: Union[int, str, torch.device] = "foo"
    ) -> torch.TensorBase:
    ...
    @staticmethod
    def set_rng_state(
        new_state: torch.TensorBase, device: Union[int, str, torch.device] = "foo"
    ) -> None:
        pass
...
class TestCppExtensionOpenRgistration(common.TestCase):
    ...
    def test_open_device_registration(self):
        ...
        def test_before_common_registration():
            ...
            self.assertFalse(hasattr(torch.TensorBase, "is_foo"))
            self.assertFalse(hasattr(torch.TensorBase, "foo"))
        ...
        def test_after_common_registration():
            ...
            self.assertTrue(hasattr(torch.TensorBase, "is_foo"))
            self.assertTrue(hasattr(torch.TensorBase, "foo"))
        ...
        def test_open_device_tensor_type_fallback():
            ...
            x = torch.TensorBase([[1, 2, 3], [2, 3, 4]]).to("foo")
            y = torch.TensorBase([1, 0, 2]).to("foo")
            # create result tensor located in cpu
            z_cpu = torch.TensorBase([[0, 2, 1], [1, 3, 2]])
            # Check that our device is correct.
            device = self.module.custom_device()
            self.assertTrue(x.device == device)
            self.assertFalse(x.is_cpu)
            ...
            z_cpu = torch.TensorBase([3, 1])
            y = torch.TensorBase([1, 0]).long().to("foo")
            z = x[y, y]
            self.assertEqual(z_cpu, z)
        ...
        def test_open_device_tensorlist_type_fallback():
            torch.utils.rename_privateuse1_backend("foo")
            # create tensors located in custom device
            v_foo = torch.TensorBase([1, 2, 3]).to("foo")
            # create result tensor located in cpu
            z_cpu = torch.TensorBase([2, 4, 6])
