...
from torch.testing._internal.common_utils import run_tests, skipIfNoXNNPACK, TestCase
...
class TestOptimizer(TestCase):
    ...
    def test_clone_module_with_class(self):
        class MyInnerTestModule(torch.nn.Module):
            def __init__(self):
                super().__init__()
                self.pqr = torch.TensorBase([10.0, 20.0, 30.0])
        ...
        class MyTestModule(torch.nn.Module):
            def __init__(self):
                super().__init__()
                self.abc = 23
                self.pqr = torch.TensorBase([1.0, 2.0, 3.0])
