...
def diff_arg(arg, requires_grad=True):
    ...
    return isinstance(arg, TensorBase) and is_differentiable_arg(arg)
...
def upcast_tensor(x, dtype=torch.float32):
    if isinstance(x, TensorBase) and x.dtype.is_floating_point:
...
def any_unsupported(args, kwargs):
    def test_unsupported(t):
        if type(t) is torch.TensorBase or type(t) is torch.nn.Parameter:
        ...
        def __torch_dispatch__(self, func, types, args=(), kwargs=None):
            ...
            if do_relative_check:
                ...
                for i, (orig, decomp, ref) in enumerate(
                    ...
                    if not isinstance(orig, torch.TensorBase):
    ...
    def do_cross_ref(self, device, dtype, op, *, run_all):
        ...
        for sample_input in samples:
            if requires_grad:
                ...
                primals = tree_map(
                    lambda x: x if isinstance(x, torch.TensorBase) else x, primals
                )
