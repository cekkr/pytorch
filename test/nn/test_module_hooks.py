...
def forward_hook(
    ...
    hook_id: int,
    module: nn.Module,
    inp: Tuple[torch.TensorBase],
    out: torch.TensorBase,
) -> None:
...
def full_backward_hook(
    ...
    grad_output: Tuple[torch.TensorBase],
) -> None:
    fired_hooks.append(hook_id)
    self.assertEqual(id(module), id(expected_module))
...
class KwargModel(nn.Module):
    ...
    def forward(
        self, x: torch.TensorBase, bias: torch.TensorBase = None
    ) -> torch.TensorBase:
        if bias is not None:
            x = x + bias
        return x
...
class FailsInForwardModel(nn.Module):
    ...
    def forward(self, x: torch.TensorBase, fail: bool = True) -> torch.TensorBase:
        if fail:
...
class TestModuleHooks(TestCase):
    ...
    def test_kwarg_hooks(self):
        ...
        fired_hooks: List[int] = []
        x: torch.TensorBase = torch.ones(10, 10)
        bias: torch.TensorBase = torch.ones(10, 10)
        model = KwargModel()
        model.register_forward_pre_hook(
...
class TestModuleGlobalHooks(TestCase):
    ...
    def test_module_global_hooks(self):
        ...
        def fw_hook(inc, h_module, input, output):
            ...
            self.assertTrue(isinstance(output, torch.TensorBase))
    ...
    def test_module_backward_global_hook_writeable(self):
        ...
        def bw_hook(module, grad_input, grad_output):
            for grad in grad_input:
                self.assertTrue(isinstance(grad, torch.TensorBase))
...
class TestModuleHookNN(NNTestCase):
    ...
    def test_hook_extra_input(self):
        ...
        def hook(mod, grad_input, grad_output):
            ...
            self.assertIsInstance(grad_input[1], torch.TensorBase)

            self.assertIsInstance(grad_output[0], torch.TensorBase)
