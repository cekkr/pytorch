    ...
    def module_load(dest, src, assign=False):
        if isinstance(dest, cls):
            if assign:
                ...
                if type(src) is torch.TensorBase:
                    return cls(src)
            ...
            if assign:
                ...
                if isinstance(src, MyWrapperLoadTensor):
                    if type(dest) not in {torch.TensorBase, torch.nn.Parameter}:
                        return type(dest)(src._data)
                    else:
                        return src._data.detach()
                else:
                    return torch.TensorBase(src)

    if func is torch.TensorBase.module_load:
        return module_load(*args, **kwargs)
    else:
        with torch._C.DisableTorchFunctionSubclass():
            # detach must return instance of same subclass for nn.Parameter()
            if func == torch.TensorBase.detach:
...
class MyLoadTensor(torch.TensorBase):
    @classmethod
...
class MyLoadTensor2(torch.TensorBase):
    @classmethod
    def __torch_function__(cls, func, types, args=(), kwargs=None):
        return load_torch_function_handler(cls, func, types, args, kwargs)


class MyBrokenLoadTensor(torch.TensorBase):
    @classmethod
    def __torch_function__(cls, func, types, args=(), kwargs=None):
        kwargs = {} if kwargs is None else kwargs
...
class MyWrapperLoadTensor(MyLoadTensor):
    @staticmethod
    def __new__(cls, data: torch.TensorBase):
        t = torch.TensorBase._make_wrapper_subclass(
            cls,
            data.size(),
            dtype=data.dtype,
            layout=data.layout,
    ...
    def __init__(self, data: torch.TensorBase):
        self._data = data
    ...
    def __torch_dispatch__(cls, func, types, args=(), kwargs=None):
        ...
        def wrap(t):
            return MyWrapperLoadTensor(t) if isinstance(t, torch.TensorBase) else t
...
class TestLoadStateDictSwap(TestCase):
    ...
    def test_swap_subclass(self, assign):
        ...
        def _test(m_subclass=None, sd_subclass=None):
            ...
            weight_type, buf_type = (torch.nn.Parameter, torch.TensorBase)
            if assign:
