...
def assert_ref_meta_equal(test_case, func, meta_rs, rs, msg_callable):
    ...
    for i, meta_r, r in zip(range(len(flat_rs)), flat_meta_rs, flat_rs):
        ...
        if not isinstance(r, torch.TensorBase):
            continue
        test_assert(
            isinstance(meta_r, torch.TensorBase), f"but real {i}th result is Tensor"
        )
        test_assert(
...
def run_meta_crossref(
    ...
    *,
    ...
    if do_meta and to_meta.successful():
        ...
        elif func is torch.TensorBase.__getitem__:
...
meta_function_skips = {
    torch.TensorBase.__rmatmul__: {bf16, c128, f64, f32, f16, c64},
    torch.TensorBase.matmul: {f64, f32, c128, c64},
    ...
    torch.TensorBase.addbmm_: {bf16, c128, c64, f32, f64, i16, i32, i64, i8, u8},
...
meta_dispatch_early_skips = set(
    {
        ...
        torch.TensorBase.cumprod_,
        torch.TensorBase.cumsum_,
    }
)
...
def get_strided_args(args):
    ...
    for arg in args:
        if (
            isinstance(arg, torch.TensorBase)
...
class TestMeta(TestCase):
    ...
    def test_meta_outplace(self, device, dtype, op):
        ...
        for sample_input in samples:
            ...
            with MetaCrossRefFunctionMode(
                ...
                if isinstance(expected, torch.TensorBase) and op.supports_out:
            ...
            if "device" in kwargs and "_like" in op.name:
                ...
                assert isinstance(args[0], torch.TensorBase)
                with torch.random.fork_rng():
                    torch.manual_seed(123)
    ...
    def _run_dispatch_meta_test(
        ...
        for sample_input in samples:
            ...
            if (
                all_stride_variants
                and sum(isinstance(arg, torch.TensorBase) for arg in sample_args) <= 5
            ):
            ...
            for args in strided_args:
                with MetaCrossRefDispatchMode.push(
                    ...
                    if (
                        not inplace
                        and isinstance(expected, torch.TensorBase)
                        and op.supports_out
                    ):
                        func(*args, **kwargs, out=expected)
    ...
    def test_inplace_bin_ops_error(self):
        t = torch.randn(3, 3, device="meta")
        for op in (
            torch.TensorBase.add_,
            torch.TensorBase.sub_,
            torch.TensorBase.mul_,
            torch.TensorBase.div_,
            torch.TensorBase.logical_and_,
            torch.TensorBase.logical_or_,
            torch.TensorBase.logical_xor_,
        ):
            with self.assertRaisesRegex(RuntimeError, "doesn't match the broadcast"):
