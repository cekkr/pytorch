    ...
    def check_function_with_fake(self, fn):
        ...
        for a, b in zip(pytree.tree_leaves(out), pytree.tree_leaves(out_fake)):
            if not isinstance(a, torch.TensorBase):
                self.assertTrue(not isinstance(b, torch.TensorBase))
                continue
...
[torch.storage.TypedStorage(dtype=torch.float32, device=meta) of size 3]""",
    ...
    def test_embedding_bag_private(self):
        ...
        with FakeTensorMode() as m:
            meta_args = [
                m.from_tensor(a) if isinstance(a, torch.TensorBase) else a for a in args
            ]
    ...
    def test_cross_entropy_loss(self):
        ...
        for w in (weight, None):
            ...
            with FakeTensorMode() as m:
                meta_args = [
                    m.from_tensor(a) if isinstance(a, torch.TensorBase) else a
                    for a in args
                ]
    ...
    def test_fake_tensor_prop_on_nn_module(self):
        ...
        with FakeTensorMode() as fake_tensor_mode:
            ...
            def to_fake_tensor(x):
                if isinstance(x, torch.TensorBase) and not isinstance(x, FakeTensor):
