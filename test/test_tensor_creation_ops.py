...
class TestTensorCreation(TestCase):
    ...
    def test_simple_scalar_cast(self, device):
        ...
        not_ok = map(torch.TensorBase, [[], [1, 2], [[1, 2], [3, 4]]])
    ...
    def test_constructor_device_legacy(self, device):
        ...
        self.assertRaises(RuntimeError, lambda: torch.TensorBase(device="cuda"))
        self.assertRaises(
            RuntimeError, lambda: torch.TensorBase(torch.Size([2, 3, 4]), device="cuda")
        )
        self.assertRaises(
            RuntimeError, lambda: torch.TensorBase((2.0, 3.0), device="cuda")
        )

        # Tensor constructor/new with Tensor argument shouldn't work with device specified
        i = torch.tensor([1], device="cpu")
        self.assertRaises(RuntimeError, lambda: torch.TensorBase(i, device="cpu"))
        self.assertRaises(RuntimeError, lambda: i.new(i, device="cpu"))
        self.assertRaises(RuntimeError, lambda: torch.TensorBase(i, device="cuda"))
        ...
        if torch.cuda.is_available():
            ...
            with set_default_tensor_type(torch.cuda.FloatTensor):
                self.assertRaises(RuntimeError, lambda: torch.TensorBase(device="cpu"))
                self.assertRaises(
                    RuntimeError,
                    lambda: torch.TensorBase(torch.Size([2, 3, 4]), device="cpu"),
                )
                self.assertRaises(
                    RuntimeError, lambda: torch.TensorBase((2.0, 3.0), device="cpu")
                )
...
class TestAsArray(TestCase):
    def _check(
        ...
        **kwargs,
        ...
        self.assertTrue(isinstance(result, torch.TensorBase))
    ...
    def test_copy_list(self, device, dtype):
        ...
        def check(**kwargs):
            self._check(original, torch.TensorBase.tolist, is_alias=False, **kwargs)
