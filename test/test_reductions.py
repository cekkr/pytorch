...
class TestReductions(TestCase):
    ...
    def test_result_dtype(self, device, dtype, op: ReductionOpInfo):
        ...
        t = make_tensor((5,), dtype=dtype, device=device)
        args, kwargs = next(op.generate_args_kwargs(t))
        result: torch.TensorBase = op(t, *args, **kwargs)
    ...
    def _test_noncontiguous(
        self, op: ReductionOpInfo, t: torch.TensorBase, **reduction_kwargs
    ):
    ...
    def _test_ref(self, op: ReductionOpInfo, t: torch.TensorBase, **reduction_kwargs):
    ...
    def _make_tensors(
        ...
        use_integral=True,
        use_complex=False,
    ) -> Dict[str, List[torch.TensorBase]]:
        ...
        def make_contiguous(shape, dtype) -> torch.TensorBase:
        ...
        def make_non_contiguous(shape, dtype) -> torch.TensorBase:
        ...
        def make_contiguous_slice(size, dtype) -> torch.TensorBase:
        ...
        if use_complex:
            types += complex_types
        tensors: Dict[str, List[torch.TensorBase]] = {
            "cont": [],
            "noncont": [],
            "slice": [],
        }
